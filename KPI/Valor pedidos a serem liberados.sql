...
WITH PEDIDOS_VLATENDPREV AS (
    SELECT NUMPED,
           SUM ((
               CASE
                   WHEN QTATENDPREV >= 0 THEN VALOR
                   ELSE QTDISPONIVEL * PVENDA
               END
           )) VLATENDPREV
      FROM (
        SELECT I.NUMPED,
          ...
          FROM PCPEDI I
         INNER JOIN PCEST E ON I.CODPROD = E.CODPROD
    ...
    SELECT B.NUMPED,
           B.TIPO
      FROM PCBLOQUEIOSPEDIDO B
      LEFT JOIN MAXCODIGOBLOQUEIO M ON B.CODIGO = M.CODIGO_MAX
)
SELECT ROUND (SUM (VLTOTAL), 2) AS VLTOTAL
  FROM PCPEDC C
 INNER JOIN MOTIVOBLOQUEIO B ON C.NUMPED = B.NUMPED
 INNER JOIN PEDIDOS_VLATENDPREV P ON C.NUMPED = P.NUMPED
  LEFT JOIN PCSUPERV V ON C.CODSUPERVISOR = V.CODSUPERVISOR
 WHERE V.CODGERENTE IN (
    1, 8, 9, 10
)
   ...
   AND (C.POSICAO IN (
    ...
    OR (C.POSICAO = 'B'
   AND B.TIPO = 'C'))
   AND (TRIM (C.OBS) NOT LIKE '%FATURAR%'
    OR C.OBS IS NULL)
