WITH LANC_IMPOSTOS AS
 (SELECT L.RECNUMPRINC, L.RECNUM AS RECNUM_IMPOSTO, L.CODCONTA
   ...
   WHERE L.DTCANCEL IS NULL --NAO CONSIDERAR LANCAMENTOS CANCELADOS*/
     ...
     AND L.NUMNOTA <> 0
     AND L.HISTORICO2 IN (TRIM('ISS'), TRIM('CSRF'), TRIM('IRRF'), TRIM('INSS')) --SOMENTE LANÇAMENTOS DE IMPOSTOS*/
  ),
LANC_PRINCIPAIS AS
 (SELECT I.RECNUM_IMPOSTO, L.RECNUM
    FROM PCLANC L
   INNER JOIN LANC_IMPOSTOS I ON L.RECNUM = I.RECNUMPRINC
...
SELECT DISTINCT L.CODFILIAL,
       ...
       L.HISTORICO2
  FROM PCLANC L
  LEFT JOIN LANC_PRINCIPAIS P ON L.RECNUM = P.RECNUM
  LEFT JOIN LANC_PRINCIPAIS V ON L.RECNUM = V.RECNUM_IMPOSTO
