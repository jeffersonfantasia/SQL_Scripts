...
SELECT L.ROWID,
 ...
 WHERE ( ( L.DTCOMPETENCIA > TRUNC(SYSDATE) AND L.DTPAGTO IS NOT NULL ) 
    OR ( L.VALOR < 0 AND A.MES_DTCOMPETENCIA <> MES_DTLANC AND L.NUMBANCO = 41 AND L.DTCOMPETENCIA <= L.DTLANC - 15 )
		OR EXTRACT(YEAR FROM L.DTCOMPETENCIA) < 2010
		OR EXTRACT(YEAR FROM L.DTEMISSAO) < 2010 )
   AND (L.DTESTORNOBAIXA IS NULL OR EXTRACT(YEAR FROM L.DTCOMPETENCIA) < 2010 OR EXTRACT(YEAR FROM L.DTCOMPETENCIA) > 2050)
   AND L.DTLANC >= TO_DATE('01/02/2023', 'DD/MM/YYYY') 
