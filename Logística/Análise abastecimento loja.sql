WITH ITENS_OS AS
 (SELECT OS_ID, COUNT(PROD_ID) QTITENS
    FROM (SELECT DISTINCT I.OS_ID, I.PROD_ID FROM JCENDOSITEM I)
   GROUP BY OS_ID)
SELECT CODFILIAL,
       CODFUNC,
       FUNCIONARIO,
  ...
  FROM (SELECT C.ID,
         ...
         WHERE C.TIPO_MOV = 'H'
           ...
           AND DT_INICIO >= TRUNC(SYSDATE) - 7)
 GROUP BY CODFILIAL, CODFUNC, FUNCIONARIO
