...
SELECT B.CODFILIAL,
 ...
 WHERE M.RN = 1
   ...
   AND NOT (INSTR(B.NOME, 'MOTORISTA') > 0 OR INSTR(B.NOME, 'BONIFIC') > 0 OR INSTR(B.NOME, 'COMISSAO') > 0 OR
        INSTR(B.NOME, 'EXTRAVIO') > 0 OR INSTR(B.NOME, 'CORP') > 0 OR INSTR(B.NOME, 'DEPRECIACAO') > 0 OR
        INSTR(B.NOME, 'CARTAO') > 0 OR INSTR(B.NOME, 'CONVENIO') > 0 OR INSTR(B.NOME, 'INTER') > 0)
