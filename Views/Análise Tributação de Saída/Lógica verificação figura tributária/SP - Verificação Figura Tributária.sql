...
--LUCRO REAL SP CST ICMS 00 DESTAQUE DE 13.30% E CST PIS COFINS 01
LR_SP_ICM13_PC1 AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   INNER JOIN VIEW_JC_TABELABASE_SP_ICMS13 B --FILTRAR NCM
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD
   WHERE A.UFDESTINO = 'SP'
     AND ((CASE WHEN A.REGIME_ICMS = 'S' THEN 1 ELSE 0 END) = 0 --VERIFICAR REGIME TRIBUTARIO PRODUTO
      OR (CASE WHEN A.IMPORTADO = 'N' THEN
            (CASE WHEN A.CODST = 10 THEN 1 ELSE 0 END) --SE FOR NACIONAL
           ELSE (CASE WHEN A.CODST = 11 THEN 1 ELSE 0 END) --SE FOR IMPORTADO
         END) = 0)),         
...
UNION SELECT * FROM LR_SP_ICM13_PC1
