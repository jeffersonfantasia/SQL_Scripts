/*-------------------------------------VERIFICAÇÃO DO CODST AP - LUCRO REAL E SIMPLES NACIONAL -----------------------------------*/
CREATE OR REPLACE VIEW VIEW_JC_VERIFICA_CODST_AP AS
WITH
--LUCRO REAL DIF ALIQ
LC_ST AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   INNER JOIN VIEW_JC_NCMTABELABASE_AP B --CONSIDERAR PRODUTOS COM NCM DIF ALIQ
   ...
   LEFT JOIN VIEW_JC_ST_SEM_PISCOFINS D --DESCONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 04 / 06
...
LC_SEMPISCOFINS_ST AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A    
     ...
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD
   INNER JOIN VIEW_JC_ST_SEM_PISCOFINS C --CONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 04 / 06
      ON A.CODFILIAL = C.CODFILIAL
     AND A.UFDESTINO = C.UFDESTINO
     AND A.CODPROD = C.CODPROD 
   WHERE A.UFDESTINO = 'AP'
    AND (CASE WHEN A.IMPORTADO = 'N' THEN
...
LC_NORMAL AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   LEFT JOIN VIEW_JC_NCMTABELABASE_AP B --DESCONSIDERAR PRODUTOS COM NCM DIF ALIQ
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD
   LEFT JOIN VIEW_JC_ICMS_CST40_41 C --DESCONSIDERAR PRODUTOS ISENTOS DE ICMS E PIS COFINS
...
SN_SEMPISCOFINS_ST AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_SN A
   INNER JOIN VIEW_JC_ST_SEM_PISCOFINS B --CONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 04 / 06
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD 
     AND A.CODNCMEX NOT IN ('34011190.')  
   WHERE A.UFDESTINO = 'AP'
    AND (CASE WHEN A.IMPORTADO = 'N' THEN
              (CASE WHEN A.CODST = 129 THEN 1 ELSE 0 END) --SE FOR NACIONAL
         ELSE (CASE WHEN A.CODST = 124 THEN 1 ELSE 0 END) --SE FOR IMPORTADO
...
/*-----------------------------------------VERIFICAÇÃO DO CODST AP SIMPLES NACIONAL EXCESSIVO------------------------------------------*/

CREATE OR REPLACE VIEW VIEW_JC_VERIFICA_CODST_AP_SNE AS
WITH 
