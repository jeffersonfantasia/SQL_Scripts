/*-------------------------------------VERIFICAÇÃO DO CODST AL - LUCRO REAL E SIMPLES NACIONAL -----------------------------------*/

CREATE OR REPLACE VIEW VIEW_JC_VERIFICA_CODST_AL AS
WITH
--LUCRO REAL DIF ALIQ
LC_ST AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   INNER JOIN VIEW_JC_NCMTABELABASE_AL B --CONSIDERAR PRODUTOS COM NCM DIF ALIQ
   ...
   LEFT JOIN VIEW_JC_TABELABASE_SP_ST_PC2 D --DESCONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 04
...
LC_SEMPISCOFINS_ST AS
 (SELECT A.*
   ...
   INNER JOIN VIEW_JC_NCMTABELABASE_AL B --CONSIDERAR PRODUTOS COM NCM DIF ALIQ
     ...
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD
   INNER JOIN VIEW_JC_TABELABASE_SP_ST_PC2 C --CONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 04
      ON A.CODFILIAL = C.CODFILIAL
     AND A.UFDESTINO = C.UFDESTINO
     AND A.CODPROD = C.CODPROD
   WHERE A.CODNCMEX NOT IN ('33049990.1', '34011190.') --DESCONSIDERAR O NCM EM QUESTÃO
    AND A.UFDESTINO = 'AL'
    AND (CASE WHEN A.IMPORTADO = 'N' THEN
              (CASE WHEN A.CODST = 137 THEN 1 ELSE 0 END) --SE FOR NACIONAL
...
LC_NORMAL AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   LEFT JOIN VIEW_JC_NCMTABELABASE_AL B --DESCONSIDERAR PRODUTOS COM NCM DIF ALIQ
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD
   LEFT JOIN VIEW_JC_ICMS_CST40_41 C --DESCONSIDERAR PRODUTOS ISENTOS DE ICMS E PIS COFINS
...
LC_SEMPISCOFINS AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   INNER JOIN VIEW_JC_ICMS_ST_PISCOFINS06 B --CONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 06
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD      
   FULL OUTER JOIN VIEW_JC_TABELABASE_SP_ST_PC2 C --UNIR COM PIS COFINS 04 SOMENTE NCMS FILTRADOS
      ON A.CODFILIAL = C.CODFILIAL
     AND A.UFDESTINO = C.UFDESTINO
     AND A.CODPROD = C.CODPROD
     AND A.CODNCMEX IN ('33049990.1', '34011190.')  
   WHERE A.UFDESTINO = 'AL'
    AND (CASE WHEN A.IMPORTADO = 'N' THEN
...
SN_SEMPISCOFINS_ST AS
 (SELECT A.*
   ...
   WHERE A.CODNCMEX NOT IN ('33049990.1', '34011190.') --DESCONSIDERAR O NCM EM QUESTÃO
    AND A.UFDESTINO = 'AL'
    AND (CASE WHEN A.IMPORTADO = 'N' THEN
              (CASE WHEN A.CODST = 132 THEN 1 ELSE 0 END) --SE FOR NACIONAL
...
SN_NORMAL AS
 (SELECT A.*
   ...
   LEFT JOIN VIEW_JC_ICMS_CST40_41 C --DESCONSIDERAR PRODUTOS ISENTOS DE ICMS E PIS COFINS
     ...
     AND A.UFDESTINO = C.UFDESTINO
     AND A.CODPROD = C.CODPROD    
 FULL OUTER JOIN VIEW_JC_TABELABASE_SP_ST_PC2 D --UNIR COM PIS COFINS 04 SOMENTE NCMS FILTRADOS
      ON A.CODFILIAL = D.CODFILIAL
     AND A.UFDESTINO = D.UFDESTINO
     AND A.CODPROD = D.CODPROD
     AND A.CODNCMEX IN ('33049990.1', '34011190.')
...
UNION SELECT * FROM SN_SEMPISCOFINS_ST
UNION SELECT * FROM SN_NORMAL;/


/*-----------------------------------------VERIFICAÇÃO DO CODST AL SIMPLES NACIONAL EXCESSIVO------------------------------------------*/

CREATE OR REPLACE VIEW VIEW_JC_VERIFICA_CODST_AL_SNE AS
WITH 
...
SNE_SEMPISCOFINS_ST AS
 (SELECT A.*
   ...
   INNER JOIN VIEW_JC_TABELABASE_SP_ST_PC2 B --CONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 04
     ...
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD      
   WHERE A.CODNCMEX NOT IN ('33049990.1') --DESCONSIDERAR O NCM EM QUESTÃO
    AND A.UFDESTINO = 'AL'
    AND (CASE WHEN A.IMPORTADO = 'N' THEN
              (CASE WHEN A.CODST = 137 THEN 1 ELSE 0 END) --SE FOR NACIONAL
         ELSE (CASE WHEN A.CODST = 107 THEN 1 ELSE 0 END) --SE FOR IMPORTADO
