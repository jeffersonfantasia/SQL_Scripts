...
/*LUCRO REAL DIF ALIQ*/ LC_ST AS (
     ...
     /*DESCONSIDERAR PRODUTOS ISENTOS DE ICMS E PIS COFINS*/ ON A.CODFILIAL = C.CODFILIAL
          ...
          LEFT JOIN VIEW_JC_ST_SEM_PISCOFINS D
     /*DESCONSIDERAR PRODUTOS ST ISENTOS DE ICMS E PIS COFINS 04/06*/ ON A.CODFILIAL = D.CODFILIAL
        ...
        SELECT A.*
          FROM VIEW_JC_TABELABASE_LR A
         INNER JOIN VIEW_JC_ST_SEM_PISCOFINS B
     /*CONSIDERAR PRODUTOS ST ISENTOS DE ICMS E PIS COFINS 04/06*/ ON A.CODFILIAL = B.CODFILIAL
           AND A.UFDESTINO = B.UFDESTINO
           AND A.CODPROD = B.CODPROD
         WHERE A.CODNCMEX NOT IN (
            '33049990.1'
        )
     /*DESCONSIDERAR O NCM EM QUESTÃO */
           AND A.UFDESTINO = 'RJ'
           AND (
            CASE
                WHEN A.IMPORTADO = 'N' THEN (
