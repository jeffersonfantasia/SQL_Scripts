/*-------------------------------------VERIFICAÇÃO DO CODST PE - LUCRO REAL E SIMPLES NACIONAL -----------------------------------*/

CREATE OR REPLACE VIEW VIEW_JC_VERIFICA_CODST_PE AS
WITH
--LUCRO REAL DIF ALIQ
LC_ST AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   INNER JOIN VIEW_JC_NCMTABELABASE_PE B --CONSIDERAR PRODUTOS COM NCM DIF ALIQ
   ...
   LEFT JOIN VIEW_JC_TABELABASE_SP_ST_PC3 D --DESCONSIDERAR PRODUTOS ST ISENTOS DE PIS COFINS 06
...
LC_NORMAL AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   LEFT JOIN VIEW_JC_NCMTABELABASE_PE B --DESCONSIDERAR PRODUTOS COM NCM DIF ALIQ
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD
   LEFT JOIN VIEW_JC_ICMS_CST40_41 C --DESCONSIDERAR PRODUTOS ISENTOS DE ICMS E PIS COFINS
...
LC_SEMPISCOFINS AS
 (SELECT A.*
    FROM VIEW_JC_TABELABASE_LR A
   INNER JOIN VIEW_JC_ICMS06_ST04_PISCOFINS B --CONSIDERAR PRODUTOS ICMS ISENTOS DE PIS COFINS 06 E ST ISENTO PIS COFINS 04
      ON A.CODFILIAL = B.CODFILIAL
     AND A.UFDESTINO = B.UFDESTINO
     AND A.CODPROD = B.CODPROD      
   WHERE A.UFDESTINO = 'PE'
...
/*-----------------------------------------VERIFICAÇÃO DO CODST PE SIMPLES NACIONAL EXCESSIVO------------------------------------------*/

CREATE OR REPLACE VIEW VIEW_JC_VERIFICA_CODST_PE_SNE AS
WITH 
