...
/*--CONCILIAÇÃO DO ADIANTAMENTO EM DUPLICATA--*/
    ...
    UNION ALL
/*--DEVOLUCAO ADIANTAMENTO EM DUPLICATA--*/
    SELECT B.CODFILIAL,
           ...
           B.VALOR,
           'D' AS TIPO,
           ('DEV ADIANT. CLIENTE  NF ' || B.NUMNOTADESC || ' - ' || B.CLIENTE) AS HISTORICO
      FROM PCCRECLI_BASE B
      LEFT JOIN VER_DTLANC_DTDESC_BAIXA_DUPLIC V ON B.CODIGO = V.CODIGO
     WHERE B.VALOR_ORIG > 0
...
/*--BAIXA DO ADIANTAMENTO COMO RECEITA--*/
    SELECT B.CODFILIAL,
      ...
      FROM PCCRECLI_BASE B
      LEFT JOIN VER_DTLANC_DTDESC_BAIXA_DUPLIC V ON B.CODIGO = V.CODIGO
     WHERE B.ROTINABAIXA LIKE '%619%'
       AND B.NUMTRANSBAIXA IS NOT NULL
    UNION ALL
 /*--ESTORNOS REALIZADOS---*/
    SELECT B.CODFILIAL,
           B.DTDESCONTO AS DATA,
