...
--REGISTROS CRIADOS A PARTIR DAS ROTINA DE DEVOLUÇÃO OU ROTINA 1400
 (
    ...
    SELECT B.CODFILIAL,
     ...
     WHERE (B.VALOR_ORIG > 0
      ...
      --RETIRAR LANCAMENTOS TIPO R QUE NÃO MOVIMENTARAM NUMERARIOS E NAO GERARAM LANCAMENTOS NA PCLANC
       AND (
        CASE
            WHEN B.NUMTRANSVENDADESC IS NOT NULL THEN 1
            WHEN B.ROTINABAIXA LIKE '%1286%' THEN 1
            WHEN B.NUMLANCBAIXA IS NOT NULL THEN 1
            ELSE 0
        END
    ) = 1
       AND NOT (NVL (B.ROTINAESTORNO, '0') LIKE '%1316%'
        OR NVL (B.ROTINAESTORNO, '0') LIKE '%1409%'
        OR NVL (B.ROTINAESTORNO, '0') LIKE '%ORACLE.EXE%'
        OR (NVL (B.ROTINAINSERT, '0') LIKE '%ORACLE.EXE%'
       AND B.ROTINAESTORNO IS NOT NULL)
        OR (NVL (B.ROTINAINSERT, '0') LIKE '%1286%'
       AND NVL (B.ROTINAESTORNO, '0') LIKE '%619%'
       AND NVL (B.ROTINABAIXA, '0') LIKE '%619%')
        OR (NVL (B.ROTINAINSERT, '0') LIKE '%1402%'
       AND NVL (B.ROTINAESTORNO, '0') LIKE '%619%'
       AND NVL (B.ROTINABAIXA, '0') LIKE '%619%')
        OR (NVL (B.ROTINAINSERT, '0') LIKE '%1303%'
       AND NVL (B.ROTINAESTORNO, '0') LIKE '%1402%'
       AND NVL (B.ROTINABAIXA, '0') LIKE '%1402%'))
    ...
    SELECT B.CODFILIAL,
           ...
           (
               CASE
                   WHEN (B.ROTINABAIXA LIKE '%1286%'
                       OR NVL (B.ROTINABAIXA, '1303') = '1303') THEN 'C'
                   ELSE 'D'
               END
           ) AS TIPO,
           (
               CASE
                   WHEN (B.ROTINABAIXA LIKE '%1286%'
                       OR NVL (B.ROTINABAIXA, '1303') = '1303') THEN 'BAIXA CRED NFD ' || B.NUMNOTA || ' - DUPLIC - ' || B.NUMNOTADESC
                       || ' - ' || B.CLIENTE
