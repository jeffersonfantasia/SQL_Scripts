CREATE OR REPLACE VIEW VIEW_JC_LINHACORPORATIVO AS 
  WITH CUSTO_FILIAL_SN AS
   (SELECT CODPROD,
           ...
           PERIPI AS IPI_SN,
           PERCICM AS ICMS_SN
      FROM JEFFERSON.VIEW_JC_CUSTOLIQ_CORP
     WHERE CODFILIAL = 6),
  PRECO_LUCROREAL AS
   (SELECT CODPROD,
  ...
  SELECT *
    FROM (SELECT DISTINCT P.CODFAB,
                          ...
                          SN.PRECOBRUTO_SN,
                          SN.CUSTOLIQUIDO_SN,
                          DECODE(PST.GERAICMSLIVROFISCAL, 'N', 'SIM', 'NÃO') AS ST_ES,
                          ST.UF,
                          ST.PERCDIFALIQ
            FROM PCPRODUT P
            ...
            LEFT JOIN PRODUTO_ST PST ON PST.CODPROD = P.CODPROD
