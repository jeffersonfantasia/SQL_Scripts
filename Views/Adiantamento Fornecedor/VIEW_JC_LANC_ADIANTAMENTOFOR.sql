CREATE OR REPLACE VIEW VIEW_JC_LANC_ADIANTAMENTOFOR AS
    SELECT L.CODFILIAL,
           L.RECNUM,
           L.VPAGO,
     /*SE O VALOR FOR  > 0 ENTÃO HOUVE O ADIANTAMENTO, SE FOR < 0 HOUVE DEVOLUÇÃO DO ADIANTAMENTO*/
           (
               CASE
     ...
     /*TRANSFORMAR OS VALORES NEGATIVOS PARA ARQUIVO CONTABILIDADE*/
           ...
           (
               CASE
                   WHEN L.VPAGO < 0 THEN (L.NUMTRANS || ' - ' || 'DEV ADIANT. - ' || F.FORNECEDOR || ' - ' || L.HISTORICO)
                   ELSE (L.NUMTRANS || ' - ' || 'ADIANT. - ' || F.FORNECEDOR || ' - ' || L.HISTORICO)
               END
           ) HISTORICO,
           L.NUMTRANS
      FROM PCLANC L
     ...
     /*NAO CONSIDERAR MOVIMETACOES QUE FORAM ESTORNADAS*/
