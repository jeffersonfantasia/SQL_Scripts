CREATE OR REPLACE VIEW VIEW_BI_CONTAS_RECEBER AS
         ...
         WHEN T.DTPAG IS NOT NULL THEN
          (CASE
            WHEN (TO_DATE(T.DTPAG, 'DD/MM/YYYY') -
                 TO_DATE(T.DTVENC, 'DD/MM/YYYY')) > 0 THEN
             'PAGO ATRASADO'
            ELSE
             'PAGO'
          END)
         WHEN T.DTVENC < TRUNC(SYSDATE) - 1 THEN
          'VENCIDO'
         ELSE
          'EM ABERTO'
       END) AS STATUS,
         ...
         WHEN T.DTPAG IS NOT NULL THEN
          (CASE
            WHEN (TO_DATE(T.DTPAG, 'DD/MM/YYYY') -
             ...
             0
            ELSE
             (TO_DATE(T.DTPAG, 'DD/MM/YYYY') - TO_DATE(T.DTVENC, 'DD/MM/YYYY'))
          END)
         WHEN T.DTVENC < TRUNC(SYSDATE) - 1 THEN
          (TRUNC(SYSDATE) - T.DTVENC)
         ELSE
          0
       END) AS DIAS_ATRASO,
 ...
 WHERE T.DTDESD IS NULL
   ...
   AND T.CODCOB NOT IN ('SENT', 'TR', 'CANC', 'BNF', 'ESTR', 'DEVT', 'DEVP')
