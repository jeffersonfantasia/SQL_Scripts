...
SELECT CASE
         WHEN (N.TIPODESCARGA IN ('6', '8', 'C', 'T')) THEN
          'D'
       -- Notas de Devolução
       ...
       CASE
         ...
         ELSE
          M.QTCONT * (M.PUNITCONT + NVL(M.VLIPI, 0) + NVL(M.ST, 0) +
          NVL(M.VLFRETE, 0) + NVL(M.VLOUTRASDESP, 0) -
          NVL(M.VLDESCONTO, 0) - NVL(M.VLSUFRAMA, 0))
       END VLCONTABIL,
       M.PUNITCONT,
       M.PTABELA,
       CASE
         WHEN (N.TIPODESCARGA IN ('6', '8', 'C', 'T')) THEN
          DECODE(NVL(P.FATORCONVTRIB, 0),
                 0,
                 P.UNIDADE,
         ...
         ELSE
          DECODE(NVL(P.FATORCONVTRIB, 0),
                 0,
                 (CASE
                   WHEN (P.UNIDADE <> P.UNIDADEMASTER) AND (P.QTUNITCX > 1) THEN
          ...
          ROUND(M.QTCONT *
                ...
                DECODE(NVL(N.VLDESCONTO, 0),
                        ...
                        DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOUTILIZADONFE',
                               ...
                               'B',
                               0,
                               NVL(M.VLDESCONTO, 0))) - NVL(M.VLFRETE, 0) -
                NVL(M.VLOUTRASDESP, 0) - NVL(M.VLIPI, 0) - NVL(M.ST, 0)),
                2)
         WHEN N.TIPODESCARGA IN ('N', 'F', 'I') THEN
          M.QTCONT * (M.PUNITCONT + NVL(M.VLDESCONTO, 0) - NVL(M.VLFRETE, 0) -
          NVL(M.VLOUTRASDESP, 0) - NVL(M.ST, 0))
         ELSE
          M.QTCONT * M.PUNITCONT
       END VLPRODUTO,
       CASE
         WHEN (N.TIPODESCARGA IN ('6', '8', 'C', 'T')) THEN
          ROUND(M.QTCONT * DECODE(NVL(N.VLDESCONTO, 0),
                                  0,
                                  0,
                                  DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOUTILIZADONFE',
                                                                           NVL(N.CODFILIALNF,
       ...
       CASE
         WHEN N.TIPODESCARGA IN ('6', '8', 'C', 'T') THEN
          ROUND(M.QTCONT * (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) +
                NVL(MC.VLBASEOUTROS, 0)),
                2)
         ELSE
       ...
       CASE
         WHEN N.TIPODESCARGA IN ('6', '8', 'C', 'T') THEN
          ROUND(M.QTCONT * (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) +
                NVL(MC.VLBASEOUTROS, 0)) * NVL(M.PERCICM, 0) / 100,
                2)
         ELSE
          M.QTCONT *
          NVL(MC.VLICMS, (NVL(M.BASEICMS, 0) * NVL(M.PERCICM, 0) / 100))
       END VLICMS,
       ...
       ROUND(DECODE(DECODE(MC.USAPISCOFINSLIT,
                           'S',
                           NVL(MC.QTLITRAGEM, P.LITRAGEM),
                           0),
                    0,
                    M.QTCONT * NVL(M.VLBASEPISCOFINS, 0),
                    M.QTCONT * NVL(MC.QTLITRAGEM, P.LITRAGEM) *
                    DECODE(NVL(M.VLCREDPIS, 0), 0, 0, 1)),
             2) VLBASEPISCOFINS,
       ROUND(DECODE(DECODE(MC.USAPISCOFINSLIT,
                           'S',
                           NVL(MC.QTLITRAGEM, P.LITRAGEM),
                    ...
                    DECODE(NVL(M.VLCREDPIS, 0), 0, 0, 1)),
             2) VLBASECOFINS,
       DECODE(NVL(DECODE(MC.USAPISCOFINSLIT,
                         'S',
                         NVL(MC.QTLITRAGEM, P.LITRAGEM),
                         0),
                  0),
       ...
       0 PARCELAS,
			 N.PRAZO,
			 (SELECT MAX(S.CODCOB) FROM PCNFSAID S JOIN PCMOV C ON S.NUMTRANSVENDA = C.NUMTRANSDEV WHERE C.NUMTRANSENT = N.NUMTRANSENT) CODCOB,
       'E' AS TIPOMOV
  FROM PCNFENT      N,
       PCMOV        M,
       PCPRODUT     P,
...
SELECT 'C' TIPOPROCESSO,
       ...
       (ROUND(DECODE(NVL(M.GRAVADOUNITARIO, 'N'), 'S', M.QTCONT, 1) *
               (NVL(M.VLIPI, 0) +
                DECODE(N.AGREGASTVLMERC, 'S', 0, NVL(M.VLST, 0)) +
                NVL(M.VLFRETE, 0)),
               2))) VLPRODUTO,
       ROUND(DECODE(NVL(M.GRAVADOUNITARIO, 'N'), 'S', M.QTCONT, 1) *
             NVL(M.VLDESCONTO, 0),
       ...
       0 PARCELAS,
			 ...
			 '' CODCOB,
       'E' AS TIPOMOV
  FROM PCNFENT N, PCMOVCIAP M, PCPRODCIAP P
 WHERE N.NUMTRANSENT = M.NUMTRANSENT AND P.CODPROD = M.CODPROD AND
 N.ESPECIE = 'NF' AND N.TIPODESCARGA NOT IN ('6', '7', '8', 'T') AND
 M.QTCONT > 0 AND N.VLTOTAL > 0 AND
 NVL(NVL(M.TIPOMERC, P.TIPOMERC), 'X') <> 'FR' AND N.ESPECIE = 'NF' AND
 NVL(N.MODELO, 'X') <> '22'
-----------------------------------------------------------------------------------
...
SELECT 'IC' TIPOPROCESSO,
       ...
       0 PARCELAS,
			 ...
			 '' CODCOB,
       'E' AS TIPOMOV
  FROM PCNFBASEENT L, PCNFENT N
 WHERE L.NUMTRANSENT = N.NUMTRANSENT AND L.NUMNOTA = N.NUMNOTA AND
 (L.VLTOTAL > 0 OR L.VLBASE > 0 OR L.VLICMS > 0 OR L.VLOUTRAS > 0 OR
 L.VLISENTAS > 0) AND NOT EXISTS (SELECT CODPROD
    FROM PCMOV
   WHERE NUMTRANSENT = L.NUMTRANSENT
     AND NUMNOTA = L.NUMNOTA
     AND ROWNUM = 1) AND NOT EXISTS
 (SELECT CODPROD FROM PCMOVCIAP WHERE NUMTRANSENT = L.NUMTRANSENT)
...
SELECT CASE
       ...
       (CASE
         WHEN (NVL(M.CODOPER, 'X') <> 'SD') THEN
          ROUND(M.QTCONT * (M.PUNITCONT + NVL(M.VLFRETE, 0) + NVL(M.VLOUTROS, 0)),
                2)
         ELSE
          (CASE
            WHEN CONSIDERA_OUTRASDESP = 'S' THEN
             ROUND(M.QTCONT * (ROUND(M.PUNITCONT, 6) + NVL(ROUND(M.VLOUTRASDESP,2), 0)), 2)
            ELSE
             ROUND(M.QTCONT * ROUND(M.PUNITCONT, 6), 2)
          END)
       END) AS VLCONTABIL,
       M.PUNITCONT,
       M.PTABELA,
       ...
       END QTUNITCX_FATORCONVTRIB,
       CASE
         WHEN (NVL(M.CODOPER, 'X') <> 'SDF') THEN
          ROUND((M.QTCONT *
                DECODE(MC.BONIFIC,
                        'S',
                        NVL(M.PBONIFIC, 0),
                        M.PUNITCONT + DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOUTILIZADONFE',
                                                                               NVL(M.CODFILIALNF,
                                                                                   M.CODFILIAL)),
                                                 'B'),
                                             'B',
                                             NVL(M.VLDESCONTO, 0),
                                             0) - NVL(M.ST, 0) - NVL(M.VLIPI, 0))),
                2)
         ELSE
          ROUND(M.QTCONT *
                (M.PUNITCONT + NVL(M.VLDESCONTO, 0) - NVL(M.ST, 0) -
                NVL(M.VLIPI, 0) -
                DECODE(NVL(M.CODOPER, 'X'), 'SD', NVL(M.VLFRETE, 0), 0)),
                2)
       END VLPRODUTO,
       CASE
         WHEN (NVL(M.CODOPER, 'X') <> 'SD') THEN
          ROUND(DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOUTILIZADONFE',
                                                         NVL(M.CODFILIALNF,
                                                             M.CODFILIAL)),
                           'B'),
                       'B',
       ...
       ROUND(NVL(M.QTCONT * MC.VLICMS,
                 ROUND(M.QTCONT *
                       (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) +
                       NVL(MC.VLBASEOUTROS, 0)) * NVL(M.PERCICM, 0) / 100,
                       2)),
             2) VLICMS,
       ...
       MC.EXTIPI,
       M.CODOPER,
       ROUND(DECODE(SUBSTR(M.CODECF, 1, 1),
                    'I',
                    M.QTCONT * (M.PUNITCONT - NVL(M.BASEICMS, 0)),
                    0),
             2) VLISENTAS_CF,
       ROUND(DECODE(SUBSTR(M.CODECF, 1, 1),
                    'N',
       ...
       N.CODCLI AS CODFORCLI,
			 (CASE WHEN N.CODCOB = 'CRED' THEN 1
			       WHEN N.SERIE = 'SF' AND (N.CODCOB IN ('CONV', 'D', 'PIXL') OR (SELECT CODCOBCC FROM PCCOB WHERE CODCOB = N.CODCOB) = 'CADB')
							 THEN 1
			       WHEN N.SERIE = 'SF' THEN (SELECT COUNT(E.PRESTECF) FROM PCPRESTECF E WHERE E.NUMPED = N.NUMPED AND E.CODCOB NOT IN ('CRED'))
						 ELSE (SELECT ROUND((NVL(PL.PRAZO1, 0) + NVL(PL.PRAZO2, 0) + NVL(PL.PRAZO3, 0) +
                  ...
                  DECODE(PL.NUMDIAS, 0, 1, PL.NUMDIAS))
                  FROM PCPLPAG PL WHERE PL.CODPLPAG = N.CODPLPAG)
							END)	PARCELAS,
       (CASE WHEN N.CODCOB = 'CRED' THEN 0
			       WHEN N.SERIE = 'SF' THEN (SELECT COUNT(E.PRESTECF) FROM PCPRESTECF E WHERE E.NUMPED = N.NUMPED AND E.CODCOB NOT IN ('CRED')) 
							 * NVL((SELECT MAX(PRAZOCC) FROM PCCOB WHERE CODCOB = N.CODCOB),0)
			       ELSE N.PRAZOMEDIO END) PRAZO,
			 N.CODCOB,
       'S' AS TIPOMOV
  FROM PCMOV        M,
       PCPRODUT     P,
       ...
       PCNFSAID     N,
       BASE_VLOUTRASDESP B
 WHERE PF.CODPROD(+) = M.CODPROD AND
 PF.CODFILIAL(+) = NVL(M.CODFILIALNF, M.CODFILIAL) AND
 MC.NUMTRANSITEM(+) = M.NUMTRANSITEM AND
 N.NUMTRANSVENDA(+) = M.NUMTRANSVENDA AND P.CODPROD = M.CODPROD AND
 M.CODST = T.CODST(+) AND M.QTCONT > 0 AND M.STATUS IN ('A', 'AB') AND
 M.NUMTRANSVENDA = B.NUMTRANSVENDA (+) AND
 DECODE(NVL(MC.BONIFIC, 'X'), 'S', M.PBONIFIC, M.PUNITCONT) > 0
-----------------------------------------------------------------------------------
...
SELECT 'C' TIPOPROCESSO,
       ...
       0 PRAZO,
			 '' CODCOB,
       'S' AS TIPOMOV
  FROM PCMOVCIAP M, PCPRODCIAP P
 WHERE P.CODPROD = M.CODPROD AND
 (M.VLITEM > 0 OR M.BASEICMS > 0 OR M.VLICMS > 0 OR M.VLFRETE > 0 OR
 M.VLOUTRASDESP > 0) AND NOT EXISTS
 (SELECT CODPROD
    FROM PCMOV
   WHERE NUMTRANSVENDA = M.NUMTRANSVENDA
     AND NUMNOTA = M.NUMNOTA
...
SELECT 'IC' TIPOPROCESSO,
       ...
       N.CODCLI AS CODFORCLI,
			 0 PARCELAS,
       N.PRAZOMEDIO AS PRAZO,
			 N.CODCOB,
       'S' AS TIPOMOV
  FROM PCNFBASESAID L, PCNFSAID N
 WHERE L.NUMTRANSVENDA = N.NUMTRANSVENDA AND L.NUMNOTA = N.NUMNOTA AND
 (L.VLTOTAL > 0 OR L.VLBASE > 0 OR L.VLICMS > 0 OR L.VLOUTRAS > 0 OR
 L.VLISENTAS > 0) AND NOT EXISTS
 (SELECT CODPROD
    FROM PCMOV
