CREATE OR REPLACE VIEW VIEW_JC_CUSTOLIQ_CORP AS
    WITH TRIBUTACAO_ENTRADA AS (
        ...
        SELECT ROUND ((C.PLIQCOMPRA * (1 - C.PRECICMS_ESCOLHIDO) + (C.PLIQCOMPRA * C.PERIPI)), 6) AS BASEPISCOFINS_SEM_ICMS,
               ROUND ((C.PLIQCOMPRA + (C.PLIQCOMPRA * C.PERIPI)), 6) AS BASEPISCOFINS,
