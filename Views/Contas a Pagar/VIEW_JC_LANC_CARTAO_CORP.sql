CREATE OR REPLACE VIEW VIEW_JC_LANC_CARTAO_CORP AS
    SELECT B.CODFILIAL,
           ...
           (
               CASE
                   WHEN NVL (B.CODROTINABAIXA, 0) IN (638) THEN 'E'
                   WHEN (B.DTESTORNOBAIXA IS NOT NULL AND B.VPAGO > 0 AND B.CODCONTA = 590101) THEN 'CE'
                   ELSE 'C'
               END
           ) AS TIPO,
           B.HISTORICO
     ...
     /*SOMENTE LANCAMENTOS DE PAGAMENTO DAS PARCELAS RETIRANDO ESTORNOS DO LANÇAMENTO DE DESPESA*/
        ...
        OR (B.DTESTORNOBAIXA IS NOT NULL AND B.VPAGO > 0 AND B.CODCONTA = 590101))
