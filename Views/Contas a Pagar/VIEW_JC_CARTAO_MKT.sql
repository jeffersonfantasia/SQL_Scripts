CREATE OR REPLACE VIEW VIEW_JC_CARTAO_MKT AS
  SELECT L.CODFILIAL,
   ...
   WHERE L.DTCANCEL IS NULL --NAO CONSIDERAR LANCAMENTOS CANCELADOS
     ...
     AND (M.CODBANCO IN (40, 41, 72, 75) OR
         (M.CODBANCO IN (36, 39) AND L.CODCONTA IN (3201, 410104)) OR
         (M.CODBANCO = 11 AND L.CODCONTA IN (3201, 620106)))
  ...
  SELECT L.CODFILIAL,
    ...
    LEFT JOIN PCFORNEC F ON L.CODFORNEC = F.CODFORNEC
   WHERE L.CODCONTA IN (3201, 410104) --SOMENTE LANCAMENTOS DAS NOTAS DE COMISSAO DE MKT
  ;
  /
