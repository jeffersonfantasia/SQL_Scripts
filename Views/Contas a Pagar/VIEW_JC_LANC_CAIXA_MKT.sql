...
WITH REGISTRO_UNICO_ESTORNADO AS
 (SELECT M.NUMSEQ
    FROM PCMOVCR M JOIN PCLANC L ON L.NUMTRANS = M.NUMTRANS
   WHERE ((M.ESTORNO = 'N' AND M.CODROTINALANC = 631) OR
        (M.ESTORNO = 'S' AND M.CODROTINALANC = 604 AND M.DTESTORNO IS NOT NULL) OR
        (M.ESTORNO = 'S' AND M.CODROTINALANC = 631 AND M.ROTINALANCAMENTO LIKE '[PCSIS604%'))
...
SELECT *
  FROM (SELECT DISTINCT L.CODFILIAL,
         ...
         WHERE NVL(L.INDICE, 0) NOT IN ('B') --LANÇAMENTOS ORIUNDOS DA MOVIMETAÇÃO DE ESTOQUE E CUSTO
           ...
           AND L.CODCONTA IN (670103, 620105)
