...
/*NAO CONSIDERAR LANCAMENTOS CANCELADOS*/
 WHERE L.DTCANCEL IS NULL
   ...
   AND (L.TIPOLANC = 'C' OR L.DTPAGTO IS NOT NULL)
      /*RETIRAR DESPESAS QUE JÁ ESTEJAM NO LIVRO FISCAL*/
   AND (D.CODFORNEC IS NULL AND D.NUMNOTA IS NULL)
   AND L.TIPOPARCEIRO = 'F'
      /*RETIRAR LANCAMENTOS DE ADIANTAMENTO FORNECEDOR*/
   AND L.CODCONTA NOT IN (200133, 200139)
   AND L.VALOR > 0
   AND ((L.CODCONTA IN (2266, 2262, 2267, 810101, 810102, 810103) OR F.OBS2 = 'DESP') OR (L.CODCONTA IN (3206, 140109) AND F.OBS2 = 'GOV'));
