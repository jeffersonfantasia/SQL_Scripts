CREATE OR REPLACE VIEW VIEW_JC_CARTAO_MKT AS
    SELECT L.CODFILIAL,
           ...
           (
               CASE
                   WHEN M.CODBANCO = 41 THEN 'NFS ' || L.NUMNOTA || ' - ' || L.HISTORICO || ' - ' || F.FORNECEDOR
                   ELSE 'NFS ' || L.NUMNOTA || ' - ' || F.FORNECEDOR
               END
           ) AS HISTORICO,
           (
               CASE
     ...
     /*NAO CONSIDERAR MOVIMENTACOES QUE FORAM ESTORNADAS*/
       ...
       AND L.CODCONTA = 410104)
        OR (M.CODBANCO = 11
       AND L.CODCONTA = 620106)) 
   /*SOMENTE LANÇAMENTOS NOS BANCOS DE COMISSAO MKT E CARTÃO DE CREDITO OU NOTAS DA AMAZON DE ASSINATURA OU NOTAS DE GATEWAY DA PAGAR.ME*/
    ...
    SELECT L.CODFILIAL,
      ...
      LEFT JOIN PCFORNEC F ON L.CODFORNEC = F.CODFORNEC
     WHERE L.CODCONTA = 410104;
     /*SOMENTE LANCAMENTOS DAS NOTAS DE COMISSAO DE MKT  */
/