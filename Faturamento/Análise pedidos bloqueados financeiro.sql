...
TITULOS AS
 (SELECT C.CODCLIREDE, T.DTPAG, T.DTVENC, T.VALOR, T.TXPERM
    FROM PCPREST T
   INNER JOIN CLIENTES C ON T.CODCLI = C.CODCLI
   INNER JOIN VENDEDORES V ON T.CODUSUR = V.CODUSUR),
TITULOS_ABERTO AS
 (SELECT T.CODCLIREDE, MIN(T.DTVENC) DT_A_VENCER, SUM(T.VALOR + NVL(T.TXPERM,0)) VL_A_VENCER
    FROM TITULOS T
...
TITULOS_VENCIDOS AS
 (SELECT T.CODCLIREDE, MIN(T.DTVENC) DT_VENCIDO, SUM(T.VALOR + NVL(T.TXPERM,0)) VL_VENCIDO
    FROM TITULOS T
   INNER JOIN DIAS_UTEIS D ON T.DTVENC = D.DATA
   WHERE T.DTPAG IS NULL
     AND D.DTVENC_UTIL < TRUNC(SYSDATE)
