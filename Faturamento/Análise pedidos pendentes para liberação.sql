...
SELECT CODFILIAL,
  ...
  FROM (SELECT DISTINCT C.CODFILIAL,
               ...
               T.CLIENTE,
               C.OBS,
               SUM(C.VLTOTAL) OVER (PARTITION BY C.CODFILIAL, C.CODCLI, C.OBS) SOMA_VLTOTAL,
               SUM(A.VLATENDPREV) OVER (PARTITION BY C.CODFILIAL, C.CODCLI, C.OBS) SOMA_VLATENDPREV,
               SUM(CA.VLCREDITO) OVER (PARTITION BY C.CODFILIAL, C.CODCLI, C.OBS) VLCREDITO          
          FROM PCPEDC C
