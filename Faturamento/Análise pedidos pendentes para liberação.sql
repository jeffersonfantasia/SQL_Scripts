SELECT B.*
  FROM ---------------TABELA "B" CRIADA PARA TER A SOMA DO VALOR PREVISTO ATENDIDO E A MENOR DATA POR CLIENTE--------------------3-----
       (SELECT C.CODFILIAL,
               MIN(C.DATA) MENOR_DATA,
               T.CODCLIPRINC,
               C.CODCLI,
               T.CLIENTE,
               C.OBS,
               SUM(C.VLTOTAL) SOMA_VLTOTAL,
               SUM(A.VLATENDPREV) SOMA_VLATENDPREV
         ...
         GROUP BY C.CODFILIAL, T.CODCLIPRINC, C.CODCLI, T.CLIENTE, C.OBS) B
-------------------------------------------------------------------------------------------------3-----
 WHERE CODFILIAL IN (2)
   AND SOMA_VLATENDPREV >= 500
 ORDER BY MENOR_DATA;

SELECT C.POSICAO, C.CODUSUR, C.CODCOB, C.*
  FROM PCPEDC C
 WHERE NUMPED = 6807;