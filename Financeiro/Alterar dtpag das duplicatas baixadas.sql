...
SELECT P.CODFILIAL,
 ...
 WHERE TO_NUMBER(EXTRACT(YEAR FROM M.DTCOMPENSACAO) || LPAD(EXTRACT(MONTH FROM M.DTCOMPENSACAO),2,'0')) <>
   ...
   AND M.CODCOB = 'D'
   AND TO_NUMBER(EXTRACT(YEAR FROM P.DTPAG) ||  LPAD(EXTRACT(MONTH FROM P.DTPAG),2,'0')) >= 202401
  ...
  SELECT P.NUMTRANSVENDA, P.PREST, P.DTPAG, M.DTCOMPENSACAO
   ...
   WHERE TO_NUMBER(TO_CHAR(M.DTCOMPENSACAO, 'YYYY') ||
     ...
     AND TO_NUMBER(TO_CHAR(P.DTPAG, 'YYYY') || TO_CHAR(P.DTPAG, 'MM')) >
         202401) X ON (P.NUMTRANSVENDA = X.NUMTRANSVENDA AND P.PREST = X.PREST) WHEN MATCHED THEN UPDATE SET P.DTPAG = X.DTCOMPENSACAO;
/
--CONTAS A PAGAR
  SELECT --L.ROWID,
   L.CODFILIAL,
