...
SELECT L.ROWID,
 ...
 WHERE L.CODROTINABAIXA IN (631, 638, 749) AND L.CODCONTA IN ( 650102, 650104)
   AND L.NUMNOTA = 0
 ORDER BY L.DTLANC;
...
SELECT P.ROWID, C.CLIENTE, P.PRESTTEF, P.NUMTRANSVENDA, P.NUMTRANS, P.OBSTITULO,P.NSUHOST, P.NSUTEF, P.DTTRANSACAOCC, P.NUMRESUMO, P.PDV, P.CODBAIXACARTAOREDE, P.DTFECHA, P.*
 ...
 WHERE P.NUMTRANSVENDA IN (11418123)
 ORDER BY DUPLIC, TO_NUMBER(PREST);
 
 SELECT P.ROWID, C.CLIENTE, P.PRESTTEF, P.NUMTRANSVENDA, P.NUMTRANS, P.OBSTITULO,P.NSUHOST, P.NSUTEF, P.DTTRANSACAOCC, P.NUMRESUMO, P.PDV, P.CODBAIXACARTAOREDE, P.DTFECHA, P.*
  FROM PCPREST P
  LEFT JOIN PCCLIENT C ON P.CODCLI = C.CODCLI
 WHERE --P.NSUTEF IN (262000473)
       P.NSUHOST IN (634282423)
 ORDER BY DUPLIC, TO_NUMBER(PREST);
...
SELECT * FROM PCCRECLI C WHERE C.CODCLI = 412698;
...
SELECT P.ROWID, 
  ...
  LEFT JOIN PCPRESTTEMP T ON W.NUMPEDWEB = T.NUMPEDWEB AND P.PRESTTEF = T.PRESTTEF AND CT.CODCLI = S.CODCLI
  LEFT JOIN CREDITOS_ABERTOS A ON S.NUMTRANSVENDA = A.NUMTRANSVENDA
 WHERE P.DTPAG IS NULL 
       --AND P.NUMTRANSVENDA = 11453051
...
SELECT P.DTEMISSAO,
 ...
 WHERE --P.DTPAG IS NULL
     ...
     (P.CODCLI = 148198 OR P.CODCOB IN ('PYME', 'PIX'))
  -- AND P.DTVENC < TRUNC(SYSDATE) ---1
...
CREDITOS_ABERTOS AS
 (SELECT (C.CODCLI || '-' || T.CLIENTE) CLIENTE,
   ...
   WHERE C.DTDESCONTO IS NULL)
SELECT P.DTEMISSAO,--p.numcustodia,   
       W.NUMPEDCLI,
       W.NUMPEDWEB,
       W.NUMPED,    
       P.CODCLI||' - '||C.CLIENTE CLIENTE,
 ...
 WHERE P.DTPAG IS NULL
   ...
   AND (P.CODCLI = 326041
    OR P.CODCOB IN ('RIHP') 
    OR P.CODUSUR = 55)
    --P.DTPAG IS NULL AND W.NUMPEDCLI IN ('v21687087rihp')
...
SELECT P.DTEMISSAO,
 ...
 WHERE P.DTPAG IS NULL 
  ...
  --AND W.NUMPEDCLI IN ( '274233333901')
   AND P.DTVENC < TRUNC(SYSDATE) - 1 
...
SELECT P.DTEMISSAO,
       W.NUMPEDCLI NUMPEDCLI_C,
       CT.NUMPEDCLI NUMPEDCLI_CT,
       COALESCE(W.NUMPEDCLI,CT.NUMPEDCLI) T,
       P.CODCLI||' - '||C.CLIENTE CLIENTE,
       P.NUMTRANSVENDA,
       P.CODCOB,
       P.CODUSUR,
