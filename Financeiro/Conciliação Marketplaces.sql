  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
        --AND P.DTVENC < TRUNC(SYSDATE) - 1
				--AND  W.NUMPEDCLI = 'v34997104rihp'
				--AND A.VALOR_CREDITO > 0
     AND (P.CODCLI = 326041 OR P.CODCOB IN ('RIHP') OR P.CODUSUR = 55)
   ORDER BY W.NUMPEDCLI,
            P.DTVENC,
            P.DUPLIC,
            TO_NUMBER(P.PREST); --v27593745rihp
/
...
CREDITOS_ABERTOS AS
 (SELECT (C.CODCLI || '-' || T.CLIENTE) CLIENTE,
         ...
         D.NUMTRANSVENDA
    FROM PCCRECLI C
   INNER JOIN TRANSACAO_VENDA_DEVOLVIDA D ON C.NUMTRANSENTDEVCLI = D.NUMTRANSENT
   INNER JOIN PCCLIENT T ON C.CODCLI = T.CODCLI
   WHERE C.DTDESCONTO IS NULL)
SELECT P.DTEMISSAO, P.CODFILIAL,
       --W.NUMPEDCLIMKT NUMPEDCLI,
