...
/******************************************/
  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
     AND P.DTVENC < TRUNC(SYSDATE) - 1
     AND (P.CODCLI = 326041 OR P.CODCOB IN ('RIHP') OR P.CODUSUR = 55) --AND P.DTEMISSAO >= '01/04/2022' 
...
/******************************************/
  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
        ...
        --AND P.NUMTRANSVENDA IN (11647383,11653250)
     AND P.DTVENC < TRUNC(SYSDATE) - 1
...
/******************************************/
  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
     AND P.DTVENC < TRUNC(SYSDATE) - 1
     AND (P.CODCLI = 25046 OR P.CODCOB IN ('CNOV') OR P.CODUSUR = 41)
   ORDER BY P.DTVENC, P.DUPLIC, TO_NUMBER(P.PREST);
/
/******************************************/
/*         DUPLICATAS AMAZON              */
...
/******************************************/
  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
        --(P.DTPAG IS NULL OR (P.CODCOB IN ('CRED', 'CARC')) AND NVL(P.DTPAG, TO_DATE('01/01/2021','DD/MM/YYYY')) > TO_DATE('01/01/2022','DD/MM/YYYY'))
        --AND P.NUMTRANSVENDA IN (11651365, 11653248)
     AND P.DTVENC < TRUNC(SYSDATE) - 1
     AND (P.CODCLI = 477468 OR P.CODCOB IN ('SHPE') OR P.CODUSUR = 75) --AND W.NUMPEDCLI IS NULL
   ORDER BY P.DTVENC, P.DUPLIC, TO_NUMBER(P.PREST);


/******************************************/
/*         DUPLICATAS CASA E VIDEO        */
/******************************************/
  WITH TRANSACAO_VENDA_DEVOLVIDA AS
   (SELECT NUMTRANSENT, MAX(M.NUMTRANSDEV) AS NUMTRANSVENDA
      FROM PCMOV M
  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
     AND P.DTVENC < TRUNC(SYSDATE) - 1
     AND (P.CODCLI = 477615 OR P.CODCOB IN ('CSVD') OR P.CODUSUR = 76)
   ORDER BY P.DTVENC, P.DUPLIC, TO_NUMBER(P.PREST);
...
/******************************************/
  ...
  SELECT P.DTEMISSAO,
   ...
   WHERE P.DTPAG IS NULL
     --AND P.DTVENC < TRUNC(SYSDATE) - 1
     AND (P.CODCLI = 10246 OR P.CODCOB IN ('FTLB') OR P.CODUSUR = 74)
   ORDER BY P.DTVENC, P.DUPLIC, TO_NUMBER(P.PREST);
