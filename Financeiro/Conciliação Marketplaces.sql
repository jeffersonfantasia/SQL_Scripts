...
USING (SELECT DISTINCT W.NUMTRANSVENDA,
        ...
        WHERE W.NUMTRANSVENDA IS NOT NULL
          ...
          AND NVL(LENGTH(W.NUMPEDCLI),0) < LENGTH(CT.NUMPEDCLI)) X
ON (C.NUMTRANSVENDA = X.NUMTRANSVENDA)
WHEN MATCHED THEN
  UPDATE SET C.NUMPEDCLI = X.NV_NUMPEDCLI;
/ 
...
SELECT P.ROWID, C.CLIENTE, P.PRESTTEF, P.NUMTRANSVENDA, P.NUMTRANS, P.OBSTITULO,P.NSUHOST, P.NSUTEF, P.DTTRANSACAOCC, P.NUMRESUMO, P.PDV, P.CODBAIXACARTAOREDE, P.DTFECHA, P.DTPAG, P.*
 ...
 WHERE P.NUMTRANSVENDA IN (11491295)
...
SELECT L.CODFILIAL,
 ...
 WHERE L.CODROTINABAIXA IN ( 1207, 9876 ) AND L.CODCONTA = 620103 AND CODFILIAL IN ( 1, 7, 9, 10 ) AND L.DTLANC >= TRUNC(SYSDATE) - 30;

--UPDATE DE CORREÇÃO TAXA FOI BAIXADO EM OUTRA CONTA
UPDATE PCLANC L
   SET
    L.CODCONTA = 620108
 WHERE L.CODROTINABAIXA IN ( 1207, 9876 ) AND L.CODCONTA = 620103 AND L.CODFILIAL IN ( 1, 7, 9, 10 ) AND L.DTLANC >= TRUNC(SYSDATE) - 30;

--VERIFICAR SE O CODCONTA ESTÁ ZERADO
SELECT * FROM PCLANC L WHERE CODROTINACAD = 'ESJCB9876.exe' AND CODCONTA = 0;
/
--UPDATE CODCONTA ZERADO
UPDATE PCLANC SET CODCONTA = 620108 WHERE CODROTINACAD = 'ESJCB9876.exe' AND CODCONTA = 0;
/
/******************************************/
...
SELECT P.DTEMISSAO,
 ...
 WHERE --(P.DTPAG IS NULL OR P.CODCOB = 'CRED')
   ...
   AND (P.CODCLI = 25047 
    OR P.CODCOB IN ('MERC') 
    OR P.CODUSUR = 42) AND P.CODFILIAL = '10'
 ORDER BY P.DTVENC, P.DUPLIC, TO_NUMBER(P.PREST);
/
/******************************************/
/*         DUPLICATAS RIHAPPY              */
 ...
 WHERE P.DTPAG IS NULL 
   ...
   AND (P.CODCLI = 326041
    ...
    OR P.CODUSUR = 55) --AND P.DTEMISSAO >= '01/04/2022' 
     --AND  W.NUMPEDCLI IN ('v21951523rihp')
...
SELECT P.DTEMISSAO,
 ...
 WHERE P.DTPAG IS NULL 
       --(P.DTPAG IS NULL OR (P.CODCOB IN ('CRED', 'CARC')) AND NVL(P.DTPAG, TO_DATE('01/01/2021','DD/MM/YYYY')) > TO_DATE('01/01/2022','DD/MM/YYYY'))
       --AND P.NUMTRANSVENDA = 11406767
   --AND ct.NUMPEDCLI IN ( '34440908801') 
...
SELECT P.DTEMISSAO,
 ...
 WHERE P.DTPAG IS NULL 
       ...
       --AND P.NUMTRANSVENDA = 11406767
   AND P.DTVENC < TRUNC(SYSDATE) - 1
   AND (P.CODCLI = 473516 OR P.CODCOB IN ('CGBA') OR P.CODUSUR = 70) --AND W.NUMPEDCLI IS NULL
 ORDER BY P.DTVENC, P.DUPLIC, TO_NUMBER(P.PREST);
