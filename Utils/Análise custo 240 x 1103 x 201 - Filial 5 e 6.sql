  ...
  FROM (SELECT CODFILIAL,
          ...
          FROM (SELECT E.CODFILIAL,
                 ...
                 WHERE P.CODFORNEC = D.CODFORNEC
                   ...
                   AND P.TIPOMERC = 'L'
                   AND P.REVENDA = 'S'
                   AND E.CODFILIAL IN (5, 6)
                   AND T.NUMREGIAO = 5) C
         WHERE ENVIAFV_6 = 'S' --and CODPROD IN(808869 )
           AND (PBRUTO_240 <> PROXCOMPRA_240 OR PROXCOMPRA_240 <> CUSTOFIN OR CUSTOFIN <> PVENDA_R5)) D
...
SELECT CODFILIAL,
  ...
  FROM (SELECT E.CODFILIAL,
               ...
               ROUND(NVL(T.PVENDA, 0), 4) PVENDA_R5,
               T.MARGEM
          FROM PCEST E, PCPRODUT P, PCPRODFILIAL F, PCTABPR T, PCFORNEC D
         WHERE P.CODFORNEC = D.CODFORNEC
           AND E.CODPROD = P.CODPROD
           AND F.CODPROD = P.CODPROD
           AND E.CODFILIAL = F.CODFILIAL
           ...
           AND E.CODFILIAL IN (5, 6)
           AND T.NUMREGIAO = 5) C
 WHERE CODPROD IN (810691, 810692, 810693, 810694, 810658, 809417, 807371 );
