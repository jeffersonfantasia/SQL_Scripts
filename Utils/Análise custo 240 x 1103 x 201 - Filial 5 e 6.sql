...
SELECT CODFILIAL,
  ...
  FROM (SELECT E.CODFILIAL,
          ...
          FROM BROKER.PCEST E, BROKER.PCPRODUT P, BROKER.PCPRODFILIAL F, BROKER.PCFORNEC D
 ...
 WHERE ENVIAFV_6 = 'S'
   AND ((PBRUTO_240 <> PROXCOMPRA_240 AND
       (ABS(PBRUTO_240 - PROXCOMPRA_240) > 0.01 OR PROXCOMPRA_240 = 0)) OR
       NVL(CUSTOFIN, 0) = 0)
  ...
  SELECT CODFILIAL,
   ...
   WHERE ENVIAFV_6 = 'S'
     AND ((PBRUTO_240 <> PROXCOMPRA_240 AND
         (ABS(PBRUTO_240 - PROXCOMPRA_240) > 0.01 OR PROXCOMPRA_240 = 0)) OR
         NVL(CUSTOFIN, 0) = 0)
     AND NVL(PROXCOMPRA_240, 0) <> 0) X ON (E.CODPROD = X.CODPROD AND E.CODFILIAL = X.CODFILIAL) 
		 WHEN MATCHED 
			 THEN UPDATE SET E.CUSTOREAL = X.PROXCOMPRA_240, 
				 E.CUSTOFIN = X.PROXCOMPRA_240, 
				 E.CUSTOREP = X.PROXCOMPRA_240, 
				 E.CUSTOCONT = X.PROXCOMPRA_240;

/*Análise custo 240 x 1103 x 201 - Filial 2 e 3*/
SELECT CODFILIAL,
       DTCADASTRO,
       CODFORNEC,
       FORNECEDOR,
  ...
  FROM (SELECT E.CODFILIAL,
         ...
         WHERE P.CODFORNEC = D.CODFORNEC
           ...
           AND P.TIPOMERC = 'L'
           AND P.REVENDA = 'S'
           AND E.CODFILIAL IN ( '2', '3')) C
 WHERE ENVIAFV_6 = 'S'
  AND ((CUSTOPROXIMA_3 <> PROXCOMPRA_240 OR PROXCOMPRA_240 = 0) 
	    OR (NVL(CUSTOFIN, 0) = 0) AND NVL(PROXCOMPRA_240, 0) <> 0)
 ORDER BY FORNECEDOR, CODPROD;

--UPDATE NA Filial 2 e 3--
MERGE
  INTO PCEST E
  USING (
  SELECT CODFILIAL,
 ...
 WHERE ENVIAFV_6 = 'S'
   AND ((CUSTOPROXIMA_3 <> PROXCOMPRA_240 OR PROXCOMPRA_240 = 0) 
	    OR (NVL(CUSTOFIN, 0) = 0) AND NVL(PROXCOMPRA_240, 0) <> 0)) X --INSERIR O CODIGO DO PRODUTO
   ON (E.CODPROD = X.CODPROD AND E.CODFILIAL = X.CODFILIAL)
	 WHEN MATCHED 
		 THEN UPDATE 
			 SET E.CUSTOREAL = X.CUSTOPROXIMA_3, 
			     E.CUSTOFIN = X.CUSTOPROXIMA_3, 
					 E.CUSTOREP = X.PROXCOMPRA_240, 
					 E.CUSTOCONT = X.CUSTOPROXIMA_3;
