  ...
  SELECT CODFILIAL,
   ...
   WHERE ENVIAFV_6 = 'S'
     AND ((PBRUTO_240 <> PROXCOMPRA_240 AND
         (ABS(PBRUTO_240 - PROXCOMPRA_240) > 0.01 OR PROXCOMPRA_240 = 0)) OR
         NVL(CUSTOFIN, 0) = 0)
     AND NVL(PROXCOMPRA_240, 0) <> 0) X ON (E.CODPROD = X.CODPROD AND E.CODFILIAL = X.CODFILIAL) WHEN MATCHED THEN UPDATE SET E.CUSTOREAL = X.PROXCOMPRA_240, E.CUSTOFIN = X.PROXCOMPRA_240, E.CUSTOREP = X.PROXCOMPRA_240, E.CUSTOCONT = X.PROXCOMPRA_240;
/
  SELECT CODFILIAL,
         DTCADASTRO,
         CODFORNEC,
    ...
    FROM (SELECT E.CODFILIAL,
                 ...
                 (SELECT ROUND(CUSTOPROXIMACOMPRA, 4)
                   ...
                   WHERE CODPROD = E.CODPROD
                     AND CODFILIAL = 3) CUSTOPROXIMA_3,
                 (SELECT ROUND((NVL(CUSTOREP, 0) * ((100 - PERCDESC) / 100)),
                               4)
                    FROM PCPRODFILIAL
                   WHERE CODPROD = E.CODPROD
                     AND CODFILIAL = 3) PBRUTO_FIL_3,
...
MERGE
  ...
  SELECT CODFILIAL,
   ...
   ON (E.CODPROD = X.CODPROD AND E.CODFILIAL = '2') WHEN MATCHED THEN UPDATE SET E.CUSTOREAL = X.PROXCOMPRA_240, E.CUSTOFIN = X.PROXCOMPRA_240, E.CUSTOREP = X.PROXCOMPRA_240, E.CUSTOCONT = X.PROXCOMPRA_240;
/
