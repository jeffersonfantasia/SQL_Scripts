...
SELECT P.ROTINAPAG, P.ROTINAPAG,
       P.Codadmcartao,
       P.CODCOBORIG,
       P.CODBANCO,
       C.CLIENTE,
 ...
 WHERE P.NUMTRANSVENDA IN (22428217)
 ORDER BY DUPLIC,
          TO_NUMBER(PREST);
...
SELECT P.ROTINAPAG,
  ...
  FROM PCPREST P
  LEFT JOIN PCCLIENT C ON P.CODCLI = C.CODCLI
 WHERE P.NUMRESUMO = '185503495'
 ORDER BY DUPLIC,
          TO_NUMBER(PREST);
					
	--UPDATE PCPREST SET DTTRANSACAOCC = '04/07/2026' WHERE DTTRANSACAOCC IS NULL AND NUMRESUMO = '017949823';
	--UPDATE PCPREST SET NUMRESUMO = NULL WHERE DTPAG IS NOT NULL AND NUMRESUMO = '017949823';
...
SELECT (C.CODCLI || '-' || T.CLIENTE) CLIENTE,
 ...
 WHERE 1=1
   AND C.CODFILIAL = '13'
	 --AND D.NUMTRANSVENDA IS NULL
   --AND T.CODCLIPRINC = 326041
...
SELECT P.ROWID,
 ...
 WHERE 1 = 1
   AND P.DTPAG IS NULL
   AND P.NUMTRANSVENDA IN (22423713); --18/09/2025 - FAVOR VERIFICAR A FALTA DE RECEBIMENTO ATÉ O MOMENTO

--UPDATE OBSERVACAO TITULO
UPDATE PCPREST
   SET OBSTITULO = '28/04/2026 - FALTA DEVOLUCAO - FAVOR VERIFICAR'
 WHERE DTPAG IS NULL
   AND NUMTRANSVENDA IN (22420029);
...
USING (SELECT NUMTRANSVENDA,
        ...
        WHERE VPAGO IS NULL
          AND PRESTTEF = 0
          AND P.NUMTRANSVENDA IN (22204559)
        ORDER BY DUPLIC,
                 TO_NUMBER(PREST)) X
ON (P.NUMTRANSVENDA = X.NUMTRANSVENDA AND P.PREST = X.PREST)
WHEN MATCHED THEN
