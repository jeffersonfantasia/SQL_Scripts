...
SELECT L.ROWID,
 ...
 WHERE L.NUMNOTA = 9424
   /*AND L.CODFORNEC = 9284*/ AND L.VALOR > 0 AND L.DTESTORNOBAIXA IS NULL;
/
/*CONTA--*/
SELECT ROWID,
       C.CONTACONTABIL,
...
SELECT T.NUMTRANS,
       ...
       T.*
  FROM PCPREST T
 WHERE ( T.DUPLIC = 56448 AND CODFILIAL = 2 ) OR T.NUMTRANS = 178728
...
SELECT T.NUMTRANS,
       ...
       T.*
  FROM PCPREST T
 WHERE T.DUPLIC = 58038 AND CODFILIAL = 2
 ORDER BY TO_NUMBER(T.PREST);
/
SELECT T.NUMTRANS,
       T.DTESTORNO,
...
SELECT ROWID,
       ...
       L.*
  FROM PCLANC L
 WHERE VALOR = 8215.01 AND CODFORNEC = 9745;
/
SELECT ROWID,
       L.TIPOPARCEIRO,
       L.DTCOMPETENCIA,
...
SELECT L.*
  FROM PCLANC L
 WHERE L.CODROTINABAIXA = 631 AND L.DTLANC = TRUNC(SYSDATE) - 1;
/
SELECT L.*
  FROM PCLANC L
...
SELECT ROWID,
       ...
       L.*
  FROM PCLANC L
 WHERE CODFORNEC = 9712 AND DTEMISSAO >= '01-JUN-2020';
/ 
...
SELECT ROWID,
       ...
       L.*
  FROM PCLANC L
 WHERE CODFORNEC = 349 AND DTEMISSAO >= '01-JUN-2020';
/
SELECT ROWID,
       L.DTPAGTO,
       L.*
  FROM PCLANC L
 WHERE L.NUMTRANS IN ( 188023 );
/
SELECT ROWID,
       L.CODFORNECPRINC,
       L.CODFORNEC,
...
SELECT S.CODCOB,
       S.*
  FROM PCNFSAID S
 WHERE CODFILIAL = 7 AND NUMNOTA IN ( 155142 );
/
/*NOTAS DE ENTRADA*/
SELECT *
  FROM PCNFENT
...
SELECT ROWID,
       ...
       P.*
  FROM PCPEDC P
 WHERE CONDVENDA = 7 AND POSICAO NOT IN ( 'F', 'C' ) AND CONTAORDEM IS NULL;
/
SELECT NUMPEDRCA,
       POSICAO,
       P.*
...
SELECT ROWID,
       ...
       P.*
  FROM PCPEDC P
 WHERE CODUSUR = 27 AND CONDVENDA = 10
 ORDER BY DATA DESC;
/
/*CREDITO CLIENTE*/
SELECT *
  FROM PCCRECLI C
 WHERE C.NUMTRANS IN ( 152422, 152425 );
/
SELECT ROWID,
       C.*
  FROM PCCRECLI C
 WHERE C.CODCLI IN ( 602 );
/
SELECT *
  FROM PCREDECLIENTE
 WHERE CODREDE = 704;
...
SELECT DISTINCT ( F.CODPRODACAB ),
 ...
 WHERE P.TIPOMERC IN ( 'CB', 'KT' ) AND P.CODNCMEX IS NULL AND P.DTEXCLUSAO IS NULL
