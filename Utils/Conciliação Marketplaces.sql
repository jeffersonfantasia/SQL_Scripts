...
SELECT C.NUMPEDWEB, C.NUMNOTA, C.NUMTRANSVENDA, C.* FROM PCPEDC C WHERE C.NUMPEDWEB = 37489198; --C.NUMPED IN ( 4200006223,4200006738);
...
SELECT ROWID, T.IDPONTOVENDA, T.NSUTEF, T.CODAUTTEF, T.PRESTTEF, T.*
  FROM PCPRESTTEMP T
 WHERE T.NUMPEDWEB = 37489198;

--DUPLICATAS ESTORNADA ANALÍTICO
SELECT C.CLIENTE, P.PRESTTEF, P.ROWID, P.*
  FROM PCPREST P
  LEFT JOIN PCCLIENT C ON P.CODCLI = C.CODCLI
 WHERE P.NUMTRANSVENDA = 11247819 --627427757
 ORDER BY DUPLIC, TO_NUMBER(PREST);
...
SELECT P.DTEMISSAO,
  ...
  LEFT JOIN PCPEDCTEMP CT ON W.NUMPEDWEB = CT.NUMPEDWEB
  LEFT JOIN CREDITOS_ABERTOS A ON S.NUMTRANSVENDA = A.NUMTRANSVENDA
 WHERE P.DTPAG IS NULL AND P.NUMTRANSVENDA = 11339830
