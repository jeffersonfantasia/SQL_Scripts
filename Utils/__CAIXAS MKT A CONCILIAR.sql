...
WITH MOVIMENTACAO AS
 (SELECT M.CODROTINALANC, M.NUMTRANS, M.DTCOMPENSACAO
    FROM PCMOVCR M
   WHERE M.DATA = TRUNC(SYSDATE)
...
SELECT P.CODCLI,
       ...
       P.PREST,
       P.CODCOB,
       P.DTPAG, P.VPAGO, P.VALORDESC, P.VALOR,
       M.DTCOMPENSACAO,
       M.CODROTINALANC
  FROM PCPREST P
  JOIN MOVIMENTACAO M ON P.NUMTRANS = M.NUMTRANS
 WHERE M.CODROTINALANC IN (1209, 1207, 9876)
...
USING (
  WITH MOVIMENTACAO AS
   (SELECT M.CODROTINALANC, M.NUMTRANS, M.DTCOMPENSACAO
     ...
     WHERE M.DATA = TRUNC(SYSDATE)
       AND M.CODBANCO = &CODBANCO)
  SELECT L.RECNUM, L.DTPAGTO, M.DTCOMPENSACAO
    FROM PCLANC L
    JOIN MOVIMENTACAO M ON L.NUMTRANS = M.NUMTRANS
   WHERE M.CODROTINALANC IN (631)
