...
---------UPDATE SHOPEE PCPEDC.NUMPEDCLI--------------
MERGE
  INTO PCPEDC C
  USING (
  SELECT DISTINCT W.NUMTRANSVENDA, CT.NUMPEDCLI NV_NUMPEDCLI
   ...
   WHERE W.NUMPEDCLI IS NULL
     ...
     AND W.DTCANCEL IS NULL
     AND CT.NUMPEDCLI IS NOT NULL
     AND (W.CODCLI = 477468 OR W.CODCOB IN ('SHPE') OR W.CODUSUR = 75)) X ON (C.NUMTRANSVENDA = X.NUMTRANSVENDA) WHEN MATCHED THEN UPDATE SET C.NUMPEDCLI = X.NV_NUMPEDCLI;

---------UPDATE CASA E VIDEO PCPEDC.NUMPEDCLI--------------
MERGE
  INTO PCPEDC C
  USING (
  SELECT DISTINCT W.NUMTRANSVENDA, CT.NUMPEDCLI NV_NUMPEDCLI
   ...
   WHERE W.NUMPEDCLI IS NULL
     ...
     AND W.DTCANCEL IS NULL
     AND CT.NUMPEDCLI IS NOT NULL
     AND (W.CODCLI = 477615 OR W.CODCOB IN ('CSVD') OR W.CODUSUR = 76)) X ON (C.NUMTRANSVENDA = X.NUMTRANSVENDA) WHEN MATCHED THEN UPDATE SET C.NUMPEDCLI = X.NV_NUMPEDCLI;
