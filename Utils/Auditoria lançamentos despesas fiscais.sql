...
SELECT E.DTLANCTO,
 ...
 WHERE E.DTCANCEL IS NULL
   AND ((E.ESPECIE = 'NF' AND B.CODFISCAL IN (1911, 1923, 2923, 1949, 2949)) OR
       (E.ESPECIE = 'NS' AND B.CODFISCAL NOT IN (1253, 1353, 2353, 1933, 2933, 1923, 2923, 1949, 2949))
			 OR (B.CODFISCAL IN (1949, 2949) AND (P.VLPIS > 0 OR P.VLCOFINS > 0) AND NVL(P.NATCREDITO,03) = 03))
   AND E.CODCONT NOT IN (200159, 100001, 100023)
   AND M.NUMTRANSENT IS NULL
   AND E.DTENT >= TO_DATE('01/06/2022', 'DD/MM/YYYY')
 ORDER BY DTENT DESC;
