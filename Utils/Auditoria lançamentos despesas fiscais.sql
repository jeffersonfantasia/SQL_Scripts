...
SELECT E.DTLANCTO,
 ...
 WHERE E.DTCANCEL IS NULL	 
	 AND ((E.ESPECIE = 'NF' AND B.CODFISCAL IN (1911, 1923, 2923, 1949, 2949)) OR
       ...
       (E.CODCONT IN (3201, 3401, 3411) AND B.CODFISCAL NOT IN (1933, 2933) OR (E.CODCONT = 3202 AND B.CODFISCAL IN (1933, 2933) AND (P.VLPIS = 0 OR P.VLCOFINS = 0))) OR
       --(E.CODCONT NOT IN (3201, 3401, 3411) AND B.CODFISCAL IN (1933, 2933)) OR
       (E.CODCONT IN (3450) AND (B.CODFISCAL NOT IN (1949, 2949) OR NVL(P.NATCREDITO, '04') <> '04' OR (P.VLPIS = 0 OR P.VLCOFINS = 0))) OR
       (E.CODCONT IN (3452) AND (B.CODFISCAL NOT IN (1253, 2353) OR NVL(P.NATCREDITO, '04') <> '04')))
   AND E.CODCONT NOT IN (200159, 100001, 100023)
   AND E.ESPECIE NOT IN ('OE')
   AND M.NUMTRANSENT IS NULL
   AND E.DTENT >= TO_DATE('01/03/2025', 'DD/MM/YYYY')
 ORDER BY DTENT DESC;
