----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (81971, 82020) ORDER BY CODCONTAB, CODFORNEC ;
SELECT F.CGC, B.* FROM BROKERCONTABIL B JOIN PCFORNEC F ON B.CODFORNEC = F.CODFORNEC WHERE B.CODCONTAB IN (81180, 1095) AND B.CODFILIAL = '1';
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (587073,587074 );
...
SELECT B.CODCLICC, B.PERCTXADMINCC, B.CODCONTACC, B.*, ROWID FROM PCCOB B WHERE B.CODCLICC IS NOT NULL AND CODCONTACC IN (9004) ORDER BY CODCOB;
SELECT ROWID, B.CODCLICC, B.PERCTXADMINCC, B.EXPORTARECF, B.* FROM PCCOB B WHERE B.EXPORTARECF = 'S' ORDER BY CODCOB;


----PCCONTA
SELECT C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 140, 210, 300,400,410,420, 510) ORDER BY C.GRUPOCONTA, CODCONTA;
SELECT rowid, C.CONTACONTABIL, C.USARATEIOCENTROCUSTO, C.* FROM PCCONTA C WHERE  C.GRUPOCONTA IN ( 210, 410 ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.CODCONTAB, F.CGC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'CORREIOS') > 0; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE REGEXP_REPLACE(F.CGC, '([0-9]{2})([0-9]{3})([0-A9]{3})([0-9]{4})', '\1.\2.\3/\4-') = '05.506.560/0001-36'; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE F.CGC = '28268233000784'; 
SELECT ROWID, F.OBS2, CODFORNECPRINC, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (9343); --48.824.508/0001-30
...
SELECT ROWID, F.OBS2, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (10621) OR F.OBS2 = 'DESP' ;
...
----PCLANC
SELECT ROWID, L.CODCONTA, L.* FROM PCLANC L WHERE L.CODCONTA IN (100003) ORDER BY L.DTPAGTO DESC ; --2260 - ICMS - SUBSTITUIÇÃO TRIBUTÁRIA
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, L. * FROM PCLANC L WHERE L.CODFILIAL IN ('7', '10' ) AND L.CODCONTA IN ( 3202, 9005) AND HISTORICO LIKE '%FRETE%' AND NUMBANCO IN (36,74) ORDER BY L.DTLANC DESC, L.CODFILIAL;
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (6977721);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, l.dtestornobaixa, l.numbanco, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 565511 ); 
SELECT ROWID, l.numtrans, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.NUMBORDERO IN ( 14834 ); 
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.NUMNOTA IN (163596) AND L.CODFORNEC = 7422;
SELECT ROWID, L.* FROM PCLANC L WHERE L.CODFILIAL IN ('1', '8','12', '13', '14') AND L.CODFORNEC IN ( 10260, 10282) AND L.DTLANC >= TRUNC(SYSDATE) - 2 AND L.CODCONTA IN (100001, 2260, 100003, 3205, 560113) ORDER BY NUMNOTA;
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.NUMBANCO, l.fornecedor, L.* FROM PCLANC L WHERE L.NUMNOTA IN ( 42108 ) AND L.CODFILIAL = '1'; 
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 9272, 72 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
SELECT ROWID, L.NUMBANCO, L.RECNUM, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODCONTA = 3206  AND DTPAGTO IS NULL; --AND DTLANC = TRUNC(SYSDATE) - 1;
SELECT L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.VPAGO = 476.33 AND L.DTPAGTO BETWEEN '01/09/2023' AND '31/10/2023'; 
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.DTESTORNOBAIXA, L.* FROM PCLANC L WHERE L.CODCONTA IN (2262) ORDER BY L.DTPAGTO DESC;
...
----PCRATEIOCENTROCUSTO
SELECT R.ROWID, R.* FROM PCRATEIOCENTROCUSTO R WHERE R.RECNUM IN (6963838, 6963835);
...
SELECT ROWID, M.CODSITTRIBPISCOFINS, M.DTCANCEL, M.DESCRICAO, M.CODFISCAL, M.NUMPED, M.ROTINALANC, M.* FROM PCMOV M WHERE M.NUMTRANSVENDA IN (10025813);
SELECT M.ROWID, M.DTCANCEL, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.PERCICMRED, M.NUMPED, M.* FROM PCMOV M WHERE M.NUMTRANSENT IN (17715 );
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (565908) ORDER BY NUMSEQ;
...
----PCNFENT
SELECT ROWID,E.CODFUNCLANC, E.FUNCLANC, E.VLFRETE, E.FORNECEDOR, E.CHAVENFE, E.TIPODESCARGA, E.DTCANCEL, E.* FROM PCNFENT E WHERE E.NUMTRANSENT IN (1800940);
...
----PCNFSAID
SELECT S.VLFRETE, S.NUMTRANSVENDA, S.CLIENTE, S.CGC, S.CHAVENFE, S.DTCANCEL, S.* FROM PCNFSAID S WHERE S.NUMTRANSVENDA IN (22196831); 
...
SELECT S.NUMTRANSVENDA, S.CLIENTE, S.CODCOB,  S.* FROM PCNFSAID S WHERE S.CODFILIAL = '11' AND S.CODCLI = 579738 AND S.DTSAIDA = '17/10/2023';
...
SELECT C.GERACP, C.CONTAORDEM, C.CONDVENDA, C.CODCOB,C.POSICAO, C.NUMPEDCLI, C.NUMTRANSVENDA,  C.* , ROWID FROM PCPEDC C WHERE C.NUMPED IN (121000004);
...
UPDATE PCPEDC SET POSICAO = 'L' WHERE POSICAO NOT IN ('C', 'F', 'L') AND  CONDVENDA = 7 AND NUMPED IN (14212502);
...
----PCPEDIDO
SELECT P.ROWID, P.CODCOMPRADOR, P.CODFILIAL, P.CODFILIALPRINC, P.* FROM PCPEDIDO P WHERE P.NUMPED IN( 221983 ); --217 VAREJO / 216 DISTRIBUIÇÃO
...
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDWEB IN ( 37287996 );
SELECT * FROM PCPEDCTEMP P WHERE INSTR(P.NUMPEDWEB, '1463990669091-01') > 0;
SELECT * FROM PCPEDCTEMP P WHERE INSTR(P.NUMPEDCLI, '1462790795573-01') > 0;
...
----PCPREST
SELECT ROWID, P.DTESTORNO,P.NUMTRANS,  P.CODBANCO, P. * FROM PCPREST P WHERE P.NUMTRANS IN (565517, 565521, 565519 );
...
SELECT ROWID, P.NUMCHEQUE,  P.NUMTRANSVENDA, P.DTESTORNO, P.* FROM PCPREST P WHERE P.CODFILIAL = '11' AND P.DUPLIC IN ( 3329  ) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST); --104479/ 104472
...
SELECT P.NSUHOST, P.NSUTEF, P.NUMTRANSVENDA, P.*, P.ROWID FROM PCPREST P WHERE P.DTEMISSAO = TRUNC(SYSDATE) - 2 AND P.CODFILIAL = '14' /*AND P.NSUHOST IS NULL*/ AND P.CODCOB NOT IN ('CRED', 'D', 'PIXL', 'CONV') ORDER BY P.NUMTRANSVENDA, P.PREST;
...
SELECT ROWID, P.CODNCMEX, P.CODFAB, P.CODFORNEC, P.OBS2, p.codauxiliar, P.COMPRACONSIGNADO, P. * FROM PCPRODUT P WHERE P.CODPROD IN (804874 );
