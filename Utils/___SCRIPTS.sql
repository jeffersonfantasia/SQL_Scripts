...
SELECT ROWID, B.* FROM BI_SINC_FORNECEDOR_CONTA B WHERE B.CODCONTABIL IN (84274,84276,83686); 
SELECT ROWID, B.* FROM BI_SINC_FORNECEDOR_CONTA B WHERE B.CODFORNEC IN (9398);
SELECT CODFORNEC, FORNECEDOR, CGC FROM PCFORNEC WHERE CODFORNEC = 10714; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (7314180,7314189);
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.RECNUMPRINC IN ( 7334963, 7334965 ) ORDER BY CODFORNEC, NUMNOTA;
...
SELECT ROWID, L.DTCOMPETENCIA,l.dtestornobaixa, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 10336 ) AND TIPOPARCEIRO = 'F'  ORDER BY L.DTPAGTO DESC;
...
SELECT m.dtcancel, M.CODST, M.NUMTRANSITEM, M.NUMTRANSENT, M.NUMTRANSVENDA, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.numnota IN ( 13298  ) AND CODFILIAL = '13';
...
SELECT M.ROWID, M.DTCANCEL, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.PERCICMRED, M.NUMPED, M.* FROM PCMOV M WHERE M.NUMTRANSENT IN (1851758 );
...
----PCMOVCOMPLE
SELECT ROWID, M.VLICMS, M.* FROM PCMOVCOMPLE M WHERE M.NUMTRANSITEM IN (104807622);
...
----PCNFENT
SELECT ROWID, E.NUMTRANSVENDAORIG, E.CODFUNCLANC, E.FUNCLANC, E.VLFRETE, E.FORNECEDOR, E.CHAVENFE, E.TIPODESCARGA, E.DTCANCEL, E.* FROM PCNFENT E WHERE E.NUMTRANSENT IN (1839970);
...
SELECT C.GERACP, C.CONTAORDEM, C.CONDVENDA, C.CODCOB,C.POSICAO, C.NUMPEDCLI, C.NUMTRANSVENDA,  C.* , ROWID FROM PCPEDC C WHERE C.NUMPED IN (157000040) OR (CODCOB IS NULL);
...
----PCPEDCTEMP
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDCLI = '1579461486701-01';
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC = TO_DATE('26/03/2026', 'DD/MM/YYYY');
