...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CODCLIPRINC, C.* FROM PCCLIENT C WHERE C.CODCLI IN (474395);
...
SELECT M.NUMTRANSENT, M.NUMTRANSVENDA, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.NUMNOTA IN (19810, 19318) AND CODFILIAL = '9';
