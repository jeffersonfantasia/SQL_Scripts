...
SELECT rowid, C.CONTACONTABIL, C.USARATEIOCENTROCUSTO, C.* FROM PCCONTA C WHERE C.TIPO = 'I' AND C.GRUPOCONTA IN ( 100 ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (6798145);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.NUMNOTA IN (12328) AND L.CODFORNEC = 7422;
...
UPDATE PCPEDC SET POSICAO = 'L' WHERE POSICAO NOT IN ('C', 'F', 'L') AND  CONDVENDA = 7 AND NUMPED IN (14212674);
UPDATE PCPEDI SET POSICAO = 'L' WHERE POSICAO NOT IN ('C', 'F', 'L') AND NUMPED IN (14212674);
