...
----PCCLIENT
SELECT C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.* FROM PCCLIENT C WHERE C.CODCLI = 465870;
...
SELECT ROWID, C.* FROM PCCRECLI C WHERE C.CODCLI = 307240 ORDER BY C.CODIGO;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (306467);
...
SELECT S.NUMTRANSVENDA, S.* FROM PCNFSAID S WHERE S.CODCLI = 465870 ORDER BY DTSAIDA DESC;
...
SELECT ROWID, C.NUMTRANSVENDA, C.NUMPEDCLI, C.* FROM PCPEDC C WHERE C.NUMPEDCLI = '701-3167728-1897844' ;
...
----PCPREST
SELECT ROWID, P.CODBANCO, P. * FROM PCPREST P WHERE P.NUMTRANS IN (305412); 
