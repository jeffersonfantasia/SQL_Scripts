...
SELECT ROWID, B.* FROM PCBANCOFILIAIS B WHERE CODBANCO = 111;
...
SELECT ROWID, C.CODIGO, C.* FROM PCCRECLI C WHERE C.CODCLI IN ( 14991 )  ORDER BY C.CODIGO;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (523559)ORDER BY NUMSEQ;
...
SELECT ROWID, M.* FROM PCMOVCR M WHERE M.NUMCARR = '16599';
--UPDATE
...
----PCNFSAID
SELECT S.VLFRETE, S.NUMTRANSVENDA, S.CLIENTE, S.CGC, S.CHAVENFE, S.DTCANCEL, S.* FROM PCNFSAID S WHERE S.NUMTRANSVENDA IN (22356245); 
...
SELECT ROWID, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.* FROM PCPEDC C WHERE C.NUMPEDCLI = '1559500509436-01' ;
SELECT ROWID, C.CODEMITENTE, C.VLFRETE, C.GERACP,  C.CONDVENDA, C.POSICAO, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.CODCOB, C.CODUSUR, C.* FROM PCPEDC C WHERE C.NUMPED IN ( 14213070,4006958 )  ORDER BY DATA DESC; 
...
----PCPEDCTEMP
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDCLI = '1559500509436-01';
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC = TO_DATE('28/12/2025', 'DD/MM/YYYY');
