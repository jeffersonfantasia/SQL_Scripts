...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE F.FORNECEDOR LIKE '%DELL%'; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 320071 ); 
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.CODFILIAL = '12' AND L.CODFORNEC = 10260 AND L.DTLANC = TRUNC(SYSDATE) - 0 AND L.CODCONTA = 100003;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (320320);
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC = TRUNC(SYSDATE);
