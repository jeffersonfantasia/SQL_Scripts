...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.CODCONTAB, F.CGC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'PAGAR.ME') > 0; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN ( 6797780);
...
----PCLOGLANCAMENTO
SELECT * FROM PCLANCLOG L WHERE L.RECNUM  IN (7129186, 7130128);
