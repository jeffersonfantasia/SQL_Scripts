...
SELECT * FROM BI_SINC_FORNECEDOR_CONTA B WHERE B.CODCONTABIL IN (3132, 3133); 
...
SELECT ROWID, C.CODIGO, C.* FROM PCCRECLI C WHERE C.CODCLI IN ( 13046 )  ORDER BY C.CODIGO;
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.CODCONTAB, F.CGC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'HIPLATAFORM') > 0; 
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (570274 )ORDER BY NUMSEQ;
