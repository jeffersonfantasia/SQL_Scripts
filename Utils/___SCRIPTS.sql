...
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (81185) ORDER BY CODCONTAB, CODFORNEC ;
...
SELECT ROWID, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (10253, 6);
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 8188 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (315085);
