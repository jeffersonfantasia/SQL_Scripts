...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODUSUR1, C.DTCADASTRO, C.* FROM PCCLIENT C WHERE C.CODCLI IN (538281);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, l.dtestornobaixa, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 319955 ); 
...
SELECT M.DTCANCEL, M.DESCRICAO, M.CODFISCAL, M.NUMPED, M.ROTINALANC, M.* FROM PCMOV M WHERE M.NUMTRANSVENDA = 21961376 ;
...
SELECT P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, P.NUMTRANS, P.* FROM PCPREST P WHERE P.NUMTRANSVENDA IN (21958167) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST);
