----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (69202) ORDER BY CODCONTAB, CODFORNEC ; --10277
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 9786 ) ;
...
SELECT ROWID, C.CODEMITENTE, C.VLFRETE, C.GERACP,  C.CONDVENDA, C.POSICAO, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.CODCOB, C.CODUSUR, C.* FROM PCPEDC C WHERE C.NUMPED IN ( 14211452 )  ORDER BY DATA DESC; 
...
SELECT ROWID, V.* FROM PCVERBA V WHERE V.NUMVERBA IN (6983);
SELECT ROWID, V.*  FROM PCAPLICVERBAI V WHERE V.NUMVERBA IN (6983);
SELECT ROWID, V.*  FROM PCAPLICVERBA V WHERE V.NUMVERBA IN (6983);
SELECT ROWID, V.*  FROM PCMOVCRFOR V WHERE V.NUMVERBA IN (6983);
