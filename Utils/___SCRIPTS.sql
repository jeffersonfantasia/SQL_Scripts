----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (70780, 65858) ORDER BY CODCONTAB, CODFORNEC ;
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 9258 ) ;
...
SELECT F.FORNECEDOR, F.CGC, B.* FROM BROKERCONTABIL B JOIN PCFORNEC F ON B.CODFORNEC = F.CODFORNEC AND B.CODFILIAL = 1 AND B.CODCONTAB IN (81068,1443,68828,69105,81313,1095,70780,70413,67714) ORDER BY B.CODCONTAB, B.CODFORNEC;
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODUSUR1, C.DTCADASTRO, C.* FROM PCCLIENT C WHERE C.CODCLI IN (474226);
...
SELECT C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 520 ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.* FROM PCFORNEC F WHERE F.FORNECEDOR LIKE '%ABRAKADABRA%'; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE REGEXP_REPLACE(F.CGC, '([0-9]{2})([0-9]{3})([0-9]{3})([0-9]{4})', '\1.\2.\3/\4-') = '41.384.562/0001-63'; 
SELECT ROWID, F.OBS2, CODFORNECPRINC, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (7543);
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, L. * FROM PCLANC L WHERE L.RECNUM IN (6630655);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 523673 ); 
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.NUMBORDERO IN ( 12818 ); 
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.NUMNOTA IN (1056) AND L.CODFORNEC = 10260;
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.NUMBANCO = 77 AND CODCONTA IN (610199) ORDER BY L.DTLANC DESC;
...
----PCMOV
SELECT M.MOVESTOQUECONTABIL M.NUMTRANSENT, M.DESCRICAO, M.* FROM PCMOV M WHERE M.NUMTRANSDEV IN (146779);
...
SELECT M.DTCANCEL, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.NUMTRANSVENDA = 21864763;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN ( 523557 );
...
----PCNFBASE
SELECT * FROM PCNFBASE B WHERE B.NUMTRANSENT = 


----PCNFBASEENT
SELECT * FROM PCNFBASEENT B WHERE B.NUMTRANSENT = 


----PCNFBASESAID
SELECT * FROM PCNFBASESAID B WHERE B.NUMTRANSENT = 
...
SELECT E.CHAVENFE, E.NUMTRANSENT, E.FORNECEDOR, E.* FROM PCNFENT E WHERE E.NUMNOTA IN (80775) ;
...
SELECT S.NUMTRANSVENDA, S.NUMPED, S.* FROM PCNFSAID S WHERE S.CODCLI = 517917 ORDER BY DTSAIDA DESC;
SELECT S.NUMTRANSVENDA, S.CLIENTE, S.CODCOB,  S.* FROM PCNFSAID S WHERE S.CODFILIAL = '7' AND S.NUMNOTA IN (359591);
...
----PCPEDC
SELECT C.NUMPED, C.NUMPEDRCA, C.NUMPEDCLI, C.NUMNOTA, C.ORIGEMPED, C.CODEMITENTE, C.NUMTRANSVENDA, C.ROWID,  C.* FROM PCPEDC C WHERE C.NUMTRANSVENDA IN ( 12840811 ); --1908165686
...
SELECT ROWID, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.* FROM PCPEDC C WHERE C.NUMPEDCLI = '701-5780750-1605837' ;
SELECT ROWID, C.CODEMITENTE, C.VLFRETE, C.GERACP,  C.CONDVENDA, C.POSICAO, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.CODCOB, C.CODUSUR, C.* FROM PCPEDC C WHERE C.NUMPED IN ( 14211185, 14211243 )  ORDER BY DATA DESC; 
SELECT C.NUMPEDRCA, C.NUMPEDCLI,  C.*, C.ROWID FROM PCPEDC C WHERE C.NUMPEDRCA IN ( 136186069, 136187734, 136187590, 136184423);
...
----PCPEDCTEMP
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDWEB LIKE '%1369790570885-01%';
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDCLI LIKE '%701-5780750-1605837%';
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDWEB IN (120528513 , 122927974);


----PCPEDI
SELECT I.ROWID, I.CODFILIALRETIRA, I.* FROM PCPEDI I WHERE I.NUMPED IN (14211185, 14211243);
...
----PCPREST
SELECT ROWID, P.DTESTORNO,P.NUMTRANS,  P.CODBANCO, P. * FROM PCPREST P WHERE P.NUMTRANS IN (521208);
...
SELECT ROWID,  P.NUMTRANSVENDA, P.DTESTORNO, P.* FROM PCPREST P WHERE P.CODFILIAL = '10' AND P.DUPLIC IN ( 3478 ) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST); --104479/ 104472
SELECT P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, P.* FROM PCPREST P WHERE P.NUMTRANSVENDA IN (12840403, 12840858) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST);
