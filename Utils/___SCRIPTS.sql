----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (66363, 66364,66366,66367,66394,10270,66410,66411) ORDER BY CODCONTAB, CODFORNEC ;
SELECT F.CGC, B.* FROM BROKERCONTABIL B JOIN PCFORNEC F ON B.CODFORNEC = F.CODFORNEC WHERE B.CODCONTAB IN (67987,82481,82755,72022,82765) AND B.CODFILIAL = '1';
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.CODCONTAB, F.CGC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'EBAZAR') > 0; 
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (570283 )ORDER BY NUMSEQ;
