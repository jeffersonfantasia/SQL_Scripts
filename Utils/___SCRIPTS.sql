...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (593053);
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN ( 7157235 , 7157260);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.NUMNOTA IN (396378, 396457, 396458, 396356, 396435) AND L.CODFORNEC = 25047;
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.CODFUNCBAIXA, L.CODFUNCULTALTER, L.ADIANTAMENTO, L.VLRUTILIZADOADIANTFORNEC, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 9547 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
SELECT M.ROWID, M.DTCANCEL, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.PERCICMRED, M.NUMPED, M.* FROM PCMOV M WHERE M.NUMTRANSENT IN (1829417 ) AND CODPROD = 825719;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (578589 )ORDER BY NUMSEQ;
...
----PCNFENT
SELECT ROWID, E.NUMTRANSVENDAORIG, E.CODFUNCLANC, E.FUNCLANC, E.VLFRETE, E.FORNECEDOR, E.CHAVENFE, E.TIPODESCARGA, E.DTCANCEL, E.* FROM PCNFENT E WHERE E.NUMTRANSENT IN (10177);
...
SELECT P.NUMTRANSVENDA, P.*, ROWID FROM PCPREST P WHERE P.CODCLI = 468891 AND P.CODFILIAL = '7' ORDER BY P.NUMTRANSVENDA, P.PREST;
...
SELECT ROWID, P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, P.NUMRESUMO, P.PRESTTEF, P.* FROM PCPREST P WHERE P.NSUHOST IN ( '' ) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST);
SELECT ROWID, P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, p.nsutef, P.PRESTTEF, P.* FROM PCPREST P WHERE P.NSUTEF IN ( '176660596' ) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST);
