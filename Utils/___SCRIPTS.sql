...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.* FROM PCFORNEC F WHERE F.FORNECEDOR LIKE '%%'; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 9626 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
SELECT * FROM PCLANC L WHERE L.NUMBORDERO = 13125;
...
SELECT M.CODST, M.NUMTRANSITEM, M.NUMTRANSENT, M.NUMTRANSVENDA, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.NUMNOTA IN (80949) AND CODFILIAL = '8';
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN ( 521676 );
...
SELECT ROWID, C.CODEMITENTE, C.VLFRETE, C.GERACP,  C.CONDVENDA, C.POSICAO, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.CODCOB, C.* FROM PCPEDC C WHERE C.NUMPED IN ( 14211135 )  ORDER BY DATA DESC; 
...
SELECT P.NSUHOST, P.NSUTEF, P.NUMTRANSVENDA, P.*, P.ROWID FROM PCPREST P WHERE P.DTEMISSAO = TRUNC(SYSDATE) - 3 AND P.CODFILIAL = '12' AND /*P.NSUHOST IS NULL AND*/ P.CODCOB NOT IN ('CRED', 'D', 'PIXL', 'CONV') ORDER BY P.NUMTRANSVENDA, P.PREST;
