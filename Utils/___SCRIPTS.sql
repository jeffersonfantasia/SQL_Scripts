...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODUSUR1, C.DTCADASTRO, C.* FROM PCCLIENT C WHERE C.CODCLI IN (523633);
...
SELECT ROWID, C.CODIGO, C.* FROM PCCRECLI C WHERE C.CODCLI IN ( 517706 )  ORDER BY C.CODIGO;
...
SELECT C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 210 ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, L. * FROM PCLANC L WHERE L.RECNUM IN (2574469, 2574478);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 319977 ); 
...
SELECT ROWID, L.* FROM PCLANC L WHERE L.CODFILIAL IN ('8','12', '13') AND L.CODFORNEC = 10260 AND L.DTLANC = TRUNC(SYSDATE) - 1 AND L.CODCONTA = 100003;
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.NUMBANCO = 15 AND CODCONTA IN (300101) ORDER BY L.DTLANC DESC;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN ( 319977, 522440, 522439 );
...
SELECT ROWID, C.NUMTRANSVENDA, C.NUMPEDCLI, C.* FROM PCPEDC C WHERE C.NUMPEDCLI = '702-1780743-4977827' ;
SELECT ROWID, C.CODEMITENTE, C.VLFRETE, C.GERACP,  C.CONDVENDA, C.POSICAO, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.CODCOB, C.CODUSUR, C.* FROM PCPEDC C WHERE C.NUMPED IN ( 3800113857 )  ORDER BY DATA DESC; 
...
----PCPEDCTEMP
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDWEB LIKE '%702-1780743-4977827%';
SELECT * FROM PCPEDCTEMP P WHERE P.NUMPEDCLI LIKE '%702-1780743-4977827%';
...
SELECT P.NUMTRANSVENDA, P.* FROM PCPREST P WHERE P.CODFILIAL = '10' AND P.DUPLIC IN ( 3520 ); --104479/ 104472
