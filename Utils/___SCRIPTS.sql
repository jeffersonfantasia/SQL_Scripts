...
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 9681,9678) ;
...
----PCFORNEC
SELECT * FROM PCFORNEC F WHERE F.FORNECEDOR LIKE '%CLARO%'; --58.731.662/0012-74
SELECT ROWID, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (9632, 10066);
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.RECNUMPRINC IN ( 2433874, 2433960) ORDER BY CODFORNEC, NUMNOTA;
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 9681,9678) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
----PCNFSAID
SELECT S.VLFRETE, S.NUMTRANSVENDA, S.CLIENTE, S.CHAVENFE, S.DTCANCEL, S.* FROM PCNFSAID S WHERE S.NUMTRANSVENDA IN (11679614); 
SELECT S.NUMTRANSVENDA, S.NUMPED, S.* FROM PCNFSAID S WHERE S.CODCLI = 7723 ORDER BY DTSAIDA DESC;
