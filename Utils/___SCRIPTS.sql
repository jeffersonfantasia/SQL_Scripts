...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (580272);
SELECT ROWID, C.CODCONTAB, C.DTULTALTER, C.CODUSUR1, C.DTCADASTRO, C.TELENT, C.TELCOB, C.TELCOM, C.EMAIL, C.* FROM PCCLIENT C WHERE C.CODCLI IN (4,7543);
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'ESCRITORIO CENTRAL') > 0; 
...
SELECT ROWID, L.CODCONTA, L.* FROM PCLANC L WHERE L.CODCONTA IN (37) ORDER BY L.DTPAGTO DESC ; --100023 - MOVIMENTACAO DE MERCADORIA
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (7002441);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 10620 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO,  M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (550036) ORDER BY NUMSEQ;
...
SELECT s.dtcancel, S.NUMTRANSVENDA, S.CLIENTE, S.CODCOB, S.SITUACAONFE, S.CHAVENFE, S.* FROM PCNFSAID S WHERE S.NUMNOTA IN (383603 ) AND S.CODFILIAL = '7'  ;
...
SELECT C.GERACP, C.CONTAORDEM, C.CONDVENDA, C.CODCOB,C.POSICAO, C.NUMPEDCLI, C.NUMTRANSVENDA,  C.* , ROWID FROM PCPEDC C WHERE C.NUMPED IN (14212351);
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC = TO_DATE('05/11/2024', 'DD/MM/YYYY');
