...
----PCCLIENT
SELECT C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.* FROM PCCLIENT C WHERE C.CODCLI = 471011;
...
SELECT L.* FROM PCLANC L WHERE L.NUMTRANS IN (304999);
...
SELECT S.NUMTRANSVENDA, S.* FROM PCNFSAID S WHERE S.CODCLI = 156064;
...
SELECT ROWID, C.NUMTRANSVENDA, C.NUMPEDCLI, C.* FROM PCPEDC C WHERE C.NUMPEDCLI = '701-7442959-7977069' ;
SELECT C.CODCOB, C.* FROM PCPEDC C WHERE C.NUMPED = 29005512;
...
----PCPREST
SELECT ROWID, P.CODBANCO, P. * FROM PCPREST P WHERE P.NUMTRANS IN (302794); 
