...
SELECT rowid, C.* FROM PCCONTA C WHERE C.CODCONTA = 3857; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, l.dtestornobaixa, l.numbanco, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 585722 ); 
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (586241)ORDER BY NUMSEQ;
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC =   TO_DATE('02/09/2025', 'DD/MM/YYYY');
