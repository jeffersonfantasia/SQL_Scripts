...
SELECT F.CGC, B.* FROM BROKERCONTABIL B JOIN PCFORNEC F ON B.CODFORNEC = F.CODFORNEC WHERE B.CODCONTAB IN (66461) AND B.CODFILIAL = '10';
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 8864 ) ;
...
SELECT ROWID, B.* FROM BI_SINC_FORNECEDOR_CONTA B WHERE B.CODCONTABIL IN (3132, 3133); 
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (4786);
...
SELECT rowid, C.CONTACONTABIL, C.USARATEIOCENTROCUSTO, C.CODCONTAMASTER, C.* FROM PCCONTA C WHERE  C.GRUPOCONTA IN ( 220 ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
SELECT ROWID, F.OBS2, CODFORNECPRINC, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (10342, 10247); --48.824.508/0001-30
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (7165402);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.NUMNOTA IN (174524) AND L.CODFORNEC = 25047;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (130009 )ORDER BY NUMSEQ;
