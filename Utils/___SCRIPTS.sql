...
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 10500 , 10470 ) ;
...
SELECT rowid, C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 555199  ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L. * FROM PCLANC L WHERE L.RECNUM IN (6971065);
...
SELECT m.dtcancel, M.CODST, M.NUMTRANSITEM, M.NUMTRANSENT, M.NUMTRANSVENDA, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.NUMNOTA IN (378566  ) AND CODFILIAL = '2' AND CODOPER = 'ED';
...
----PCPEDC
SELECT C.NUMPED, C.NUMPEDRCA, C.NUMPEDCLI, C.NUMNOTA, C.ORIGEMPED, C.CODEMITENTE, C.NUMTRANSVENDA, C.ROWID,  C.* FROM PCPEDC C WHERE C.NUMTRANSVENDA IN ( 22146831 ); --LU-1193570392354077
...
SELECT ROWID, P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, P.* FROM PCPREST P WHERE P.NSUHOST IN ( '345363414' );
