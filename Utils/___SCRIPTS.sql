...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (603659);
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, l.dtestornobaixa, l.numbanco, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 584766, 584821 ); 
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (586473 )ORDER BY NUMSEQ;
...
SELECT ROWID, P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, p.nsutef, P.PRESTTEF, P.* FROM PCPREST P WHERE P.NSUTEF IN ( '6585' ) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST);
