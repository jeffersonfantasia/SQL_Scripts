...
SELECT F.CGC, B.* FROM BROKERCONTABIL B JOIN PCFORNEC F ON B.CODFORNEC = F.CODFORNEC WHERE B.CODCONTAB IN (82020, 81971) AND B.CODFILIAL = '1';
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (527225);
...
SELECT ROWID, C.* FROM PCCRECLI C WHERE C.CODIGO IN ( 260885 ) ORDER BY C.CODIGO;
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.CODCONTAB, F.CGC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'MERCOCAMP') > 0; 
...
SELECT ROWID, M.CODSITTRIBPISCOFINS, M.DTCANCEL, M.DESCRICAO, M.CODFISCAL, M.NUMPED, M.ROTINALANC, M.* FROM PCMOV M WHERE M.NUMTRANSVENDA IN (22223512);
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (568490, 568491, 568492 )ORDER BY NUMSEQ;
...
SELECT ROWID, P.NUMCHEQUE,  P.NUMTRANSVENDA, P.DTESTORNO, P.* FROM PCPREST P WHERE P.CODFILIAL = '7' AND P.DUPLIC IN ( 367216  ) ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST); --104479/ 104472
