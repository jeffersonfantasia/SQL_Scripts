----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (6566) ORDER BY CODCONTAB, CODFORNEC ; --10277
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 10325 ) ;
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (241167);
...
SELECT rowid, C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 660 ) ORDER BY C.GRUPOCONTA, CODCONTA;
SELECT C.* FROM PCCONTA C WHERE C.CONTACONTABIL = 838; 
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.* FROM PCFORNEC F WHERE F.FORNECEDOR LIKE '%KALUNGA%'; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE REGEXP_REPLACE(F.CGC, '([0-9]{2})([0-9]{3})([0-9]{3})([0-9]{4})', '\1.\2.\3/\4-') = '41.384.562/0001-63'; 
SELECT ROWID, F.OBS2, CODFORNECPRINC, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (10506); --48.824.508/0001-30
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L. * FROM PCLANC L WHERE L.RECNUM IN (6899378);
...
SELECT M.CODST, M.NUMTRANSITEM, M.NUMTRANSENT, M.NUMTRANSVENDA, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.NUMNOTA IN (97924, 67924 ) AND CODFILIAL = '2' AND CODOPER = 'ED';
...
----PCNFENT
SELECT ROWID, E.VLFRETE, E.FORNECEDOR, E.CHAVENFE, E.TIPODESCARGA, E.* FROM PCNFENT E WHERE E.NUMTRANSENT IN (1746799, 1747417);
SELECT E.CHAVENFE, E.NUMTRANSENT, E.FORNECEDOR, E.* FROM PCNFENT E WHERE E.NUMNOTA IN ( 97924, 67924 ) AND codfilial = '2';
SELECT ROWID, E.CHAVENFE, E.NUMTRANSENT, E.FORNECEDOR, E.CODFORNECNF, E.* FROM PCNFENT E WHERE E.CODFORNEC IN (31 ) ORDER BY DTENT DESC ;
...
SELECT s.dtcancel, S.NUMTRANSVENDA, S.CLIENTE, S.CODCOB, S.SITUACAONFE,  S.* FROM PCNFSAID S WHERE S.NUMNOTA IN (64153 ) AND S.CODFILIAL = '2'  ;
...
SELECT ROWID, C.CODEMITENTE, C.VLFRETE, C.GERACP,  C.CONDVENDA, C.POSICAO, C.NUMTRANSVENDA, C.NUMNOTA, C.NUMPEDCLI, C.CODCOB, C.CODUSUR, C.* FROM PCPEDC C WHERE C.NUMPED IN ( 7400429375,7400429647  )  ORDER BY DATA DESC; 
...
----PCPEDIDO
SELECT P.ROWID, P.CODCOMPRADOR, P.* FROM PCPEDIDO P WHERE P.NUMPED IN( 219997 ); --217 VAREJO / 216 DISTRIBUIÇÃO
