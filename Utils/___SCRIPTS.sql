----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (82120, 82240) ORDER BY CODCONTAB, CODFORNEC ; --10277
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODFORNEC IN( 9955 , 10521 ) ;
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (244883);
SELECT ROWID, C.DTULTALTER, C.CODUSUR1, C.DTCADASTRO, C.TELENT, C.TELCOB, C.TELCOM, C.EMAIL, C.* FROM PCCLIENT C WHERE C.CODCLI IN (559845);
...
----PCCONTA
SELECT C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 210, 300,400,410,420, 510, 515, 520 ,525 ,530, 535, 540, 545, 550, 555, 560, 565, 610, 620, 650, 660, 820) ORDER BY C.GRUPOCONTA, CODCONTA;
SELECT rowid, C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 440  ) ORDER BY C.GRUPOCONTA, CODCONTA;
SELECT C.* FROM PCCONTA C WHERE C.CONTACONTABIL = 776; 
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'ORGANICO') > 0; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE REGEXP_REPLACE(F.CGC, '([0-9]{2})([0-9]{3})([0-9]{3})([0-9]{4})', '\1.\2.\3/\4-') = '28.268.233/0007-84'; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE F.CGC = '28268233000784'; 
SELECT ROWID, F.OBS2, CODFORNECPRINC, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (10521, 1051); --48.824.508/0001-30
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L. * FROM PCLANC L WHERE L.RECNUM IN (6947243);
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.RECNUMPRINC IN ( 1816974 ) ORDER BY CODFORNEC, NUMNOTA;
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, l.dtestornobaixa, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 557355); 
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.NUMBORDERO IN ( 14585 ); 
...
SELECT ROWID, L.* FROM PCLANC L WHERE L.CODFILIAL IN ('8','12', '13', '14') AND L.CODFORNEC IN ( 10260, 10282) AND L.DTLANC >= TRUNC(SYSDATE) - 0 AND L.CODCONTA IN (100001, 100003, 560113) ORDER BY NUMNOTA;
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.NUMBANCO, L.* FROM PCLANC L WHERE L.NUMNOTA IN ( 138163 ) AND L.CODFILIAL = '7'; 
SELECT ROWID, L.DTCOMPETENCIA, l.Codrotinaalt, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 10567 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
SELECT ROWID, L.NUMBANCO, L.RECNUM, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODCONTA = 810102 AND DTLANC = TRUNC(SYSDATE) - 1;
SELECT L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.VPAGO = 476.33 AND L.DTPAGTO BETWEEN '01/09/2023' AND '31/10/2023'; 
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.DTESTORNOBAIXA, L.* FROM PCLANC L WHERE L.CODCONTA IN (545105) ORDER BY L.DTPAGTO DESC;
...
SELECT m.dtcancel, M.CODST, M.NUMTRANSITEM, M.NUMTRANSENT, M.NUMTRANSVENDA, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.* FROM PCMOV M WHERE M.NUMNOTA IN (377863  ) AND CODFILIAL = '2' AND CODOPER = 'ED';
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO,  M.* FROM PCMOVCR M WHERE M.NUMTRANS IN ( 558526 ) ORDER BY NUMSEQ;
...
----PCNFSAID
SELECT S.VLFRETE, S.NUMTRANSVENDA, S.CLIENTE, S.CGC, S.CHAVENFE, S.DTCANCEL, S.* FROM PCNFSAID S WHERE S.NUMTRANSVENDA IN (22142424); 
...
SELECT C.CONTAORDEM, C.CONDVENDA, C.CODCOB,C.POSICAO, rowid, C.* FROM PCPEDC C WHERE C.NUMPED = 14211669;
SELECT C.GERACP, C.POSICAO, C.*, ROWID FROM PCPEDC C WHERE C.CONDVENDA = 10 AND C.POSICAO NOT IN ('C', 'F');


----PCPEDI
SELECT I.ROWID, I.CODFILIALRETIRA, I.* FROM PCPEDI I WHERE I.NUMPED IN (14211669);
...
SELECT ROWID, P.DTESTORNO,P.NUMTRANSVENDA, P.NUMTRANS,  P.CODBANCO, P. * FROM PCPREST P WHERE P.NUMTRANS IN (552654);
