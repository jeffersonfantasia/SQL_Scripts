----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (6657, 6655) ORDER BY CODCONTAB, CODFORNEC ; --10277
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (568006);
SELECT ROWID, C.DTULTALTER, C.CODUSUR1, C.DTCADASTRO, C.TELENT, C.TELCOB, C.TELCOM, C.EMAIL, C.* FROM PCCLIENT C WHERE C.CODCLI IN (220772);
...
SELECT rowid, C.CONTACONTABIL, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 545, 610, 620, 650, 660 ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'TD') > 0; 
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE REGEXP_REPLACE(F.CGC, '([0-9]{2})([0-9]{3})([0-9]{3})([0-9]{4})', '\1.\2.\3/\4-') = '28268233000784'; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L. * FROM PCLANC L WHERE L.RECNUM IN (6908950);
...
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.DTESTORNOBAIXA, L.* FROM PCLANC L WHERE L.CODCONTA IN (540199) ORDER BY L.DTPAGTO DESC;
...
SELECT M.ROWID, M.DTCANCEL, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.PERCICMRED, M.NUMPED, M.* FROM PCMOV M WHERE M.NUMTRANSENT IN (1753894);
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO,  M.* FROM PCMOVCR M WHERE M.NUMTRANS IN ( 551190 ) ORDER BY NUMSEQ;
...
SELECT S.NUMTRANSVENDA, S.NUMPED, S.* FROM PCNFSAID S WHERE S.CODCLI = 561824 ORDER BY DTSAIDA DESC;
...
SELECT P.CODAUXILIAR, P.CODPRODMASTER, P.CODPRODPRINC, P.* FROM PCPRODUT P WHERE P.DESCRICAO LIKE '%CARA A CARA%';
