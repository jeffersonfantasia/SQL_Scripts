...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (611985);
...
SELECT ROWID, F.OBS2, CODFORNECPRINC, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (9567); --48.824.508/0001-30
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (7309574, 7309568, 7309575);
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.RECNUMPRINC IN ( 7275851, 7299523 ) ORDER BY CODFORNEC, NUMNOTA;
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.NUMTRANS, L.NUMBANCO, l.fornecedor, L.* FROM PCLANC L WHERE L.NUMNOTA IN ( 23976 );-- AND L.CODFILIAL = '10'; 
SELECT ROWID, L.DTCOMPETENCIA,l.dtestornobaixa, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 10674 ) AND TIPOPARCEIRO = 'F' AND numnota = 1420 ORDER BY L.DTPAGTO DESC;
...
SELECT ROWID, P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, p.nsutef, P.PRESTTEF, P.* FROM PCPREST P WHERE P.NSUTEF IN ( '1128') ORDER BY P.NUMTRANSVENDA, TO_NUMBER(PREST);
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC = TO_DATE('20/01/2026', 'DD/MM/YYYY');
