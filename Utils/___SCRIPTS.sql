...
SELECT ROWID, L.DTCOMPETENCIA, L.DTLANC, L.DTEMISSAO, L.DTVENC, L.DTPAGTO, l.dtestornobaixa, l.numbanco, L.* FROM PCLANC L WHERE L.NUMTRANS IN ( 567287 ); 
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (567287) ORDER BY NUMSEQ;
...
SELECT C.GERACP, C.CONTAORDEM, C.CONDVENDA, C.CODCOB,C.POSICAO, C.NUMPEDCLI, C.NUMTRANSVENDA,  C.* , ROWID FROM PCPEDC C WHERE C.NUMPED IN (37288046);
