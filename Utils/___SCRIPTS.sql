...
SELECT B.CODCLICC, B.PERCTXADMINCC, B.CODCONTACC, B.*, ROWID FROM PCCOB B WHERE B.CODCLICC IS NOT NULL AND CODCONTACC IN (620108) ORDER BY CODCOB;
...
SELECT rowid, C.CONTACONTABIL, C.USARATEIOCENTROCUSTO, C.* FROM PCCONTA C WHERE C.TIPO <> 'I' AND C.GRUPOCONTA IN ( 400, 410, 420  ) ORDER BY C.GRUPOCONTA, CODCONTA;
...
SELECT ROWID, F.OBS2, F.REVENDA, F.SIMPLESNACIONAL, F.EMITENFE, F. * FROM PCFORNEC F WHERE F.CODFORNEC IN (10385); 
...
SELECT ROWID, M.CODSITTRIBPISCOFINS, M.DTCANCEL, M.DESCRICAO, M.CODFISCAL, M.NUMPED, M.ROTINALANC, M.* FROM PCMOV M WHERE M.NUMTRANSVENDA IN (11839540);
SELECT M.ROWID, M.DTCANCEL, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.PERCICMRED, M.NUMPED, M.* FROM PCMOV M WHERE M.NUMTRANSENT IN (1782513);
