...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (602599);
...
SELECT ROWID, L.DTCOMPETENCIA, L.RECNUMPRINC, l.fornecedor, L.NUMTRANS, L. * FROM PCLANC L WHERE L.RECNUM IN (7212128 );
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 9567 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO, M.DTESTORNO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (583703 )ORDER BY NUMSEQ;
...
----PCPEDIDO
SELECT P.ROWID, P.CODCOMPRADOR, P.CODFILIAL, P.CODFILIALPRINC, P.* FROM PCPEDIDO P WHERE P.NUMPED IN( 224263 ); --217 VAREJO / 216 DISTRIBUICAO
...
SELECT V.ROWID, V.* FROM PCVALECX V WHERE V.DTLANC =   TO_DATE('15/07/2025', 'DD/MM/YYYY');
