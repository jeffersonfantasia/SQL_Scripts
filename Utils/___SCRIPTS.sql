...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.* FROM PCCLIENT C WHERE C.CODCLI IN (481649);
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.* FROM PCFORNEC F WHERE F.FORNECEDOR LIKE '%MANDAE%'; --58.731.662/0012-74
...
SELECT ROWID, L.DTCOMPETENCIA, L. * FROM PCLANC L WHERE L.RECNUM IN (2428429);
SELECT ROWID, L.DTCOMPETENCIA, L.FORNECEDOR, L.RECNUMPRINC, L.* FROM PCLANC L WHERE L.RECNUMPRINC IN ( 2446479 ) ORDER BY CODFORNEC, NUMNOTA;
SELECT ROWID, L.* FROM PCLANC L WHERE L.NUMTRANS IN (312714, 312713);
...
SELECT ROWID, L.DTCOMPETENCIA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 7902, 9298) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (312332);
...
SELECT S.NUMTRANSVENDA, S.NUMPED, S.* FROM PCNFSAID S WHERE S.CODCLI = 436925 ORDER BY DTSAIDA DESC;
...
SELECT P.NUMTRANSVENDA, P.CODCOBORIG, P.OBSTITULO, P.* FROM PCPREST P WHERE P.NSUHOST IN ( '880657271' );
