----BROKERCONTABIL
SELECT B.ROWID, B.* FROM BROKERCONTABIL B WHERE B.CODCONTAB IN (66363,66364, 66365, 66366,66367, 66368, 10270 ) ORDER BY CODCONTAB, CODFORNEC ; --10277
...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (9343);
...
SELECT ROWID, C.CODIGO, C.* FROM PCCRECLI C WHERE C.CODCLI IN ( 1630 )  ORDER BY C.CODIGO;
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.CODCONTAB, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'EBAZAR') > 0; 
...
SELECT ROWID, L.DTCOMPETENCIA, L.DTESTORNOBAIXA, L.* FROM PCLANC L WHERE L.CODFORNEC IN ( 9027 ) AND TIPOPARCEIRO = 'F' ORDER BY L.DTPAGTO DESC;
...
SELECT M.ROWID, M.DTCANCEL, M.NUMTRANSDEV, M.DESCRICAO, M.CODFISCAL, M.PERCICMRED, M.NUMPED, M.* FROM PCMOV M WHERE M.NUMTRANSENT IN (1795347);
...
----PCMOVCR
SELECT ROWID, M.DTCOMPENSACAO, M.ESTORNO,  M.* FROM PCMOVCR M WHERE M.NUMTRANS IN (565511) ORDER BY NUMSEQ;
...
----PCNFBASE
SELECT B.ROWID, B.* FROM PCNFBASE B WHERE B.NUMTRANSENT = 1782513;
...
----PCNFBASEENT
SELECT * FROM PCNFBASEENT B WHERE B.NUMTRANSENT IN ( 1782513 );
...
----PCNFENT
SELECT ROWID,E.CODFUNCLANC, E.FUNCLANC, E.VLFRETE, E.FORNECEDOR, E.CHAVENFE, E.TIPODESCARGA, E.DTCANCEL, E.* FROM PCNFENT E WHERE E.NUMTRANSENT IN (1782513);
