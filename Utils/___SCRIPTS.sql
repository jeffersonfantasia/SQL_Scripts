...
----PCCLIENT
SELECT ROWID, C.CALCULAST, C.CONTRIBUINTE, C.CONSUMIDORFINAL, C.IEENT, C.CGCENT, C.CODCLIPRINC, C.CODREDE, C.CODUSUR1, C.DTCADASTRO, C.ESTENT, C.ESTCOB, C.ESTCOM, C.* FROM PCCLIENT C WHERE C.CODCLI IN (566172);
...
----PCFORNEC
SELECT F.ROWID, F.OBS2, F.REVENDA, F.TIPOFORNEC, F.* FROM PCFORNEC F WHERE INSTR(F.FORNECEDOR, 'MERCOCAMP') > 0; 
...
----PCPEDIDO
SELECT P.ROWID, P.CODCOMPRADOR, P.* FROM PCPEDIDO P WHERE P.NUMPED IN( 220315 ); --217 VAREJO / 216 DISTRIBUIÇÃO
