...
SELECT E.CODFILIAL,
  ...
  LEFT JOIN CUSTOS_FILIAL_F2 C
    ON E.CODPROD = C.CODPROD
 WHERE E.CODFILIAL = 3
   AND E.QTESTGER > 0
   AND (ROUND(E.VALORULTENT, 2) <> C.VALORULTENT_F OR
       ROUND(E.CUSTOULTENT, 2) <> C.CUSTOULTENT_F OR
       ROUND(E.CUSTOFIN, 2) <> C.CUSTOFIN_F OR
       ROUND(E.CUSTOREAL, 2) <> C.CUSTOREAL_F OR
       ROUND(E.CUSTOCONT, 2) <> C.CUSTOCONT_F OR
       ROUND(E.CUSTOREP, 2) <> C.CUSTOREP_F)
...
SELECT E.CODFILIAL,
  ...
  LEFT JOIN PCPRODUT P
    ON E.CODPROD = P.CODPROD
  LEFT JOIN CUSTOS_FILIAL_F1 C
    ON E.CODPROD = C.CODPROD
 WHERE E.CODFILIAL = 4 AND E.QTESTGER > 0 AND
 (ROUND(E.VALORULTENT, 2) <> C.VALORULTENT_F OR
 ROUND(E.CUSTOULTENT, 2) <> C.CUSTOULTENT_F OR
 ROUND(E.CUSTOFIN, 2) <> C.CUSTOFIN_F OR
 ROUND(E.CUSTOREAL, 2) <> C.CUSTOREAL_F OR
 ROUND(E.CUSTOCONT, 2) <> C.CUSTOCONT_F OR
