...
SELECT T.DTCADASTRO,
 ...
 WHERE T.CODPROD = P.CODPROD
   AND ((P.GERAICMSLIVROFISCAL IS NULL OR
       ...
       (P.GERAICMSLIVROFISCALDEVFORNEC IS NULL))
   AND T.DTCADASTRO <= TRUNC(SYSDATE)
   AND T.DTEXCLUSAO IS NULL
   AND T.CODPRODMASTER IS NOT NULL
   --AND T.TIPOMERC NOT IN ('CB')
 ORDER BY T.CODNCMEX, T.DTCADASTRO DESC, P.CODFILIAL, P.CODPROD;
 
 --UPDATE PCPRODFILIAL SET GERAICMSLIVROFISCAL = 'S' WHERE GERAICMSLIVROFISCAL IS NULL AND CODPROD IN (810608);
...
SELECT T.DTCADASTRO,
  ...
  FROM PCPRODFILIAL P, PCPRODUT T
 WHERE T.CODPROD = P.CODPROD
 AND P.CODPROD IN (810127,810128,810129,810130);
