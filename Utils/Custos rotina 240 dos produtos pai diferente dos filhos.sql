--CONSULTA PAI DIFERENTE DOS FILHOS
...
SELECT *
  FROM (SELECT F.CODFILIAL,
          ...
          FROM PCPRODFILIAL F
          JOIN PCPRODUT P ON F.CODPROD = P.CODPROD
          JOIN PCEST E ON E.CODPROD = F.CODPROD
                           AND E.CODFILIAL = F.CODFILIAL
          JOIN PRODUTO_MASTER M ON M.CODMASTER = P.CODPRODMASTER
...
USING (
  ...
  SELECT *
    FROM (SELECT F.CODFILIAL,
           ...
           WHERE P.DTEXCLUSAO IS NULL
             AND F.CODFILIAL IN ('1', '7', '8'))
   WHERE (PERCDESC <> PERCDESC_MASTER OR ABS(CUSTOPROXIMACOMPRA - PROXIMACOMPRA_MASTER) > 0.01)) X 
   ON (F.CODPROD = X.CODPROD AND F.CODFILIAL = X.CODFILIAL) 
   WHEN MATCHED THEN
    UPDATE
       SET F.CUSTOREPTAB  = X.CUSTOREP_MASTER,
           F.PERCDESC1TAB = X.PERCDESC1_MASTER,
           F.PERCDESC2TAB = X.PERCDESC2_MASTER,
           F.PERCDESC3TAB = X.PERCDESC3_MASTER,
           F.PERCDESC4TAB = X.PERCDESC4_MASTER;
/
WITH PRODUTO_MASTER AS
   (SELECT F.CODFILIAL,
           P.CODPRODMASTER CODMASTER,
