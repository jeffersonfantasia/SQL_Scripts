...
SELECT DISTINCT T.CODFILIAL,
 ...
 WHERE T.CUSTOCONT <> A.PUNITCONT OR T.CUSTOFINEST <> A.PUNITCONT;j
/
 ---UPDATE --
MERGE INTO PCMOV M
USING (
...
SELECT DISTINCT T.CODFILIAL,
  ...
  LEFT JOIN MOVENTRADA A
   ...
   AND T.CODPROD = A.CODPROD
   AND T.DTCOMPRA = A.DTMOV
 WHERE T.CUSTOCONT <> A.PUNITCONT OR T.CUSTOFINEST <> A.PUNITCONT ) X
ON (M.NUMTRANSITEM = X.NUMTRANSITEM)
WHEN MATCHED THEN
  UPDATE SET M.CUSTOCONT = X.PUNITCONT,
...
SELECT T.CODFILIAL,
  ...
  LEFT JOIN MOVENTRADA A
   ...
   AND T.CODPROD = A.CODPROD
   AND T.DTCOMPRA = A.DTMOV
 WHERE T.CUSTOCONT <> A.PUNITCONT ) X
ON (M.NUMTRANSITEM = X.NUMTRANSITEM)
WHEN MATCHED THEN
  UPDATE SET M.CUSTOCONT = X.PUNITCONT,
             M.CUSTOFINEST = X.PUNITCONT,
