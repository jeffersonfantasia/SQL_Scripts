...
WITH FORNEC_CONTABIL AS (
    ...
    SELECT DISTINCT E.CODFILIAL,
     ...
     WHERE M.CODFISCAL IN ( 1102, 2102, 1403, 1405, 2403 )
)
SELECT E.*
  FROM DESPESAS E
  LEFT JOIN FORNEC_CONTABIL B ON ( E.CODFILIAL || '-' || E.CODFORNEC ) = B.COD_ID
 WHERE E.DATA BETWEEN '01-JAN-2020' AND ADD_MONTHS(LAST_DAY(TRUNC(SYSDATE)),
                                                   - 0) AND E.GRUPOCONTA NOT IN ( 810, 820 ) AND B.COD_ID IS NULL;
/
...
SELECT *
  FROM (
    SELECT (
      ...
      LEFT JOIN FORNEC_CONTABIL B ON ( E.CODFILIAL || '-' || E.CODFORNEC ) = B.COD_ID
     WHERE E.DATA BETWEEN '01-JAN-2020' AND ADD_MONTHS(LAST_DAY(TRUNC(SYSDATE)),
                                                       - 1) AND E.GRUPOCONTA NOT IN ( 810, 820 ) AND B.COD_ID IS NULL
)
 GROUP BY FILIAL,
          CODFORNEC,
          FORNECEDOR,
...
SELECT DISTINCT F.CODFORNEC,
 ...
 WHERE NVL(B.CODFILIAL, 0) IN ( 0, 1, 2, 7, 8 ) AND F.CODFORNEC IN ( 9985 )
