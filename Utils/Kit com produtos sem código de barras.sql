...
USING (
  ...
  SELECT F.CODPRODACAB,
    ...
    LEFT JOIN PCPRODUT P ON F.CODPRODMP = P.CODPROD
    LEFT JOIN PRODUTO_ACABADO A ON F.CODPRODACAB = A.CODPROD
   WHERE (F.CODAUXILIARMP IS NULL OR F.CODAUXILIARMP <> P.CODAUXILIAR)) X 
ON (F.CODPRODACAB = X.CODPRODACAB AND F.CODPRODMP = X.CODPRODMP) 
WHEN MATCHED THEN
    UPDATE
       SET F.CODAUXILIARMP  = X.CODAUXILIAR;
----------