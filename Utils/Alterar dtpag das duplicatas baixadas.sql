...
USING (
    SELECT P.NUMTRANSVENDA,
     ...
     WHERE TO_NUMBER(TO_CHAR(M.DTCOMPENSACAO, 'YYYY') || TO_CHAR(M.DTCOMPENSACAO, 'MM')) <> TO_NUMBER(TO_CHAR(P.DTPAG, 'YYYY') || TO_CHAR(P.DTPAG, 'MM'))
       ...
       AND M.CODCOB = 'D'
       AND TO_NUMBER(TO_CHAR(P.DTPAG, 'YYYY') || TO_CHAR(P.DTPAG, 'MM')) > 202204
) X ON ( P.NUMTRANSVENDA = X.NUMTRANSVENDA
   AND P.PREST = X.PREST )
WHEN MATCHED THEN UPDATE
SET P.DTPAG = X.DTCOMPENSACAO;
/
--CONTAS A PAGAR
SELECT --L.ROWID,
 L.CODFILIAL,
 ...
 WHERE TO_NUMBER(TO_CHAR(M.DTCOMPENSACAO, 'YYYY') || TO_CHAR(M.DTCOMPENSACAO, 'MM')) <> TO_NUMBER(TO_CHAR(L.DTPAGTO, 'YYYY') || TO_CHAR(L.DTPAGTO, 'MM'))
   AND M.CODBANCO IN ( 1, 2, 5, 13, 16,
                       ...
                       44, 46, 48, 55 )
   AND M.CODCOB = 'D'
   AND L.CODCONTA NOT IN ( 610101, 610102, 620103, 620108, 650101,
                           680101 )
...
USING (
    SELECT L.CODFILIAL,
     ...
     WHERE TO_NUMBER(TO_CHAR(M.DTCOMPENSACAO, 'YYYY') || TO_CHAR(M.DTCOMPENSACAO, 'MM')) <> TO_NUMBER(TO_CHAR(L.DTPAGTO, 'YYYY') || TO_CHAR(L.DTPAGTO, 'MM'))
       ...
       AND L.DTESTORNOBAIXA IS NULL
       AND M.DTESTORNO IS NULL
) X ON ( L.RECNUM = X.RECNUM )
WHEN MATCHED THEN UPDATE
SET L.DTPAGTO = X.DTCOMPENSACAO;
/