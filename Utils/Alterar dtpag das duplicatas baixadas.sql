...
SELECT --P.ROWID,
 ...
 WHERE TO_NUMBER(TO_CHAR(M.DTCOMPENSACAO, 'YYYY') ||
       ...
       TO_NUMBER(TO_CHAR(P.DTPAG, 'YYYY') || TO_CHAR(P.DTPAG, 'MM'))
   AND M.CODBANCO IN (1, 2, 5, 13, 16, 27, 29, 31, 33, 42, 44, 46, 48, 55)
   AND P.CODCLI NOT IN (25053)
   AND M.CODCOB = 'D'
   AND TO_NUMBER(TO_CHAR(P.DTPAG, 'YYYY') || TO_CHAR(P.DTPAG, 'MM')) >
       202204
 ORDER BY P.DTPAG DESC;
---------------UPDATE PCPREST ----------------
MERGE INTO PCPREST P
USING (SELECT P.NUMTRANSVENDA, P.PREST, P.DTPAG, M.DTCOMPENSACAO
         FROM PCPREST P
        INNER JOIN PCMOVCR M ON P.NUMTRANS = M.NUMTRANS
         LEFT JOIN PCBANCO B ON M.CODBANCO = B.CODBANCO
         LEFT JOIN PCCLIENT C ON P.CODCLI = C.CODCLI
...
USING (SELECT L.CODFILIAL,
        ...
        WHERE TO_NUMBER(TO_CHAR(M.DTCOMPENSACAO, 'YYYY') ||
                        TO_CHAR(M.DTCOMPENSACAO, 'MM')) <>
              TO_NUMBER(TO_CHAR(L.DTPAGTO, 'YYYY') ||
                        TO_CHAR(L.DTPAGTO, 'MM'))
          AND M.CODBANCO IN
              (1, 2, 5, 13, 16, 27, 29, 31, 33, 42, 44, 46, 48, 55)
          AND M.CODCOB = 'D'
          AND L.CODCONTA NOT IN
              (610101, 610102, 620103, 620108, 650101, 680101)
          AND TO_NUMBER(TO_CHAR(L.DTPAGTO, 'YYYY') ||
                        TO_CHAR(L.DTPAGTO, 'MM')) > 202109
