WITH ESTOQUE_F1 AS (
    ...
    SELECT *
      FROM (
        ...
        SUM (QTVENDIDA)
        FOR CODFILIAL
        IN (1 AS "QTVENDIDA_F1", 7 AS "QTVENDIDA_F7", 8 AS "QTVENDIDA_F8")
    )
)
SELECT P.CODPROD,
       P.DESCRICAO,
       ...
       O.VLULTENT_F1,
       O.QTDISP_F1,
       NVL (V.QTVENDIDA_F1, 0) QTVENDIDA_F1,
       B.QTDISP_F8,
       NVL (V.QTVENDIDA_F8, 0) QTVENDIDA_F8,
       A.QTDISP_F2,
       F.QTDISP_F7,
       NVL (V.QTVENDIDA_F7, 0) QTVENDIDA_F7
  FROM PCEST E
 ...
 WHERE (O.QTDISP_F1 >= 1
   ...
   AND F.QTDISP_F7 <= 0
   AND A.QTDISP_F2 <= 0
 ORDER BY NVL (V.QTVENDIDA_F7, 0) DESC,
          O.QTDISP_F1 DESC;
/