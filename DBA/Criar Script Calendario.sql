WITH DATAS AS
 (SELECT (TO_DATE('01/01/2010', 'DD/MM/YYYY') + LEVEL - 1) DT
    FROM DUAL
...
SELECT DT AS DATA,
       ...
       EXTRACT(MONTH FROM DT) - EXTRACT(MONTH FROM SYSDATE) CALCULO_MES_ATUAL,
       CEIL(EXTRACT(MONTH FROM DT) / 3) -
       CEIL(EXTRACT(MONTH FROM SYSDATE) / 3) AS Diferenca_Trimestre,
       EXTRACT(YEAR FROM DT) - EXTRACT(YEAR FROM SYSDATE) AS Diferenca_Ano,
       TO_NUMBER(TO_CHAR(DT, 'WW')) - TO_NUMBER(TO_CHAR(SYSDATE, 'WW')) AS Diferenca_Semana
  FROM DATAS
