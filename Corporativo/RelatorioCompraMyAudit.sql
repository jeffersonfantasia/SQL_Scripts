...
--AGRUPAR VENDAS LIBERADAS OU MONTADAS POR PRODUTO DISTRIBUIÇAO --
QTD_RESERVADA_DISTRIBUICAO AS
 (SELECT I.CODPROD,
         SUM(NVL(I.QT, 0)) AS QT_RESERVADA_DISTRIB
    FROM PCPEDI I
    JOIN PCPEDC C ON I.NUMPED = C.NUMPED
   WHERE I.POSICAO IN ('L', 'M') --OLHAR PEDIDOS QUE NÃO ESTEJAM FATURADOS OU CANCELADO
     AND I.CODUSUR <> 14 --SOMENTE PUXAR VENDAS DO VENDEDOR CORPORATIVO
...
ESTOQUE_ATUAL AS
 (SELECT E.CODPROD,
         ...
         SUM((NVL(E.QTESTGER, 0) - NVL(R.QT_RESERVADA, 0) - NVL(E.QTINDENIZ, 0) - NVL(D.QT_RESERVADA_DISTRIB,0))) AS QTDISPONIVEL
    FROM PCEST E
		LEFT JOIN QTD_RESERVADA R ON R.CODPROD = E.CODPROD
		LEFT JOIN QTD_RESERVADA_DISTRIBUICAO D ON D.CODPROD = E.CODPROD
...
SELECT *
  FROM (SELECT T.*,
               ...
               (CASE
                 WHEN CODFORNEC = 9720
                      AND (QTBLOQCORP + QTRESERVADA) < QT_VENDIDA THEN
                  QT_VENDIDA - (QTBLOQCORP + QTRESERVADA)
                 ELSE
                  0
               END) QT_FALTA_BLOQ
          FROM RESULTADO T)
