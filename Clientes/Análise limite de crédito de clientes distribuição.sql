...
SELECT *
  FROM (SELECT DISTINCT C.CODCLI,
                        ...
                        C.DTCADASTRO,
                        C.CODCOB,
                        MAX(S.DTSAIDA) OVER(PARTITION BY C.CODCLI) DTSAIDA_CLI,
                        MAX(S.DTSAIDA) OVER(PARTITION BY C.CODCLIPRINC) DTSAIDA_CLIPRINC,
                        COUNT(S.CODCLI) OVER(PARTITION BY C.CODCLIPRINC) QTVENDA_CLIPRINC
          FROM PCCLIENT C
