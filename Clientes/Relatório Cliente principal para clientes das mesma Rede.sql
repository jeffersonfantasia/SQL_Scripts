WITH CONTAGEMREDE AS
 (SELECT CODREDE, 
         COUNT(CODREDE) AS CONT_REDE
    FROM PCCLIENT
   WHERE TIPOFJ = 'J'
   GROUP BY CODREDE),
CONTAGEMCLIPRINCIPAL AS
 (SELECT CODCLIPRINC, 
         COUNT(CODCLIPRINC) AS CONT_CLIPRINC
    FROM PCCLIENT
   WHERE TIPOFJ = 'J'
...
SELECT C.CODREDE,
  ...
  LEFT JOIN CONTAGEMREDE CR
    ON C.CODREDE = CR.CODREDE
  LEFT JOIN CONTAGEMCLIPRINCIPAL CP
    ON C.CODCLIPRINC = CP.CODCLIPRINC
 WHERE C.TIPOFJ = 'J'
   AND C.CODREDE NOT IN (0,429) --REDE ZASTRAS
