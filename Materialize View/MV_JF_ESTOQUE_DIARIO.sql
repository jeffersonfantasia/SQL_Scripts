...
TRANSF_SP AS
 (SELECT CODPROD, SUM(QT) QT
    FROM (SELECT CODPROD, QT
            FROM TRANSFERENCIA_PARA_SP
          UNION ALL
          SELECT CODPROD, QT
            FROM PEDIDO_TRANSF_SP)
   GROUP BY CODPROD),

TRANSF_ES AS
 (SELECT CODPROD, SUM(QT) QT
    FROM (SELECT CODPROD, QT
            FROM TRANSFERENCIA_PARA_ES
          UNION ALL
          SELECT CODPROD, QT
            FROM PEDIDO_TRANSF_ES)
   GROUP BY CODPROD),
...
(SELECT E.CODPROD, SUM(E.QTBLOQ) QTBLOQECOM
   FROM ESTOQUE_PRODUTO E 
   WHERE E.CODFILIAL IN ( '2', '11')
     AND NVL(E.CODDEVOL,9999) IN (43,49) 
  GROUP BY E.CODPROD),
...
SELECT P.CODPROD,
       ...
       (NVL(CP.QTCOMPRAPEND,0) - NVL(C.QTCOMPRAFAT,0)) QTCOMPRAPEND_SP,
       (NVL(CP2.QTCOMPRAPEND,0)- NVL(C2.QTCOMPRAFAT,0)) QTCOMPRAPEND_ES,
			 T.QT QTTRANSF_SP,
       T2.QT QTTRANSF_ES,
       E1.QTDISP QTDISP_SP,
       E2.QTDISP QTDISP_ES,
  ...
  LEFT JOIN PRODUTOS_COMPRADOS_PENDENTES_ES CP2 ON CP2.CODPROD = P.CODPROD
	LEFT JOIN TRANSF_SP T ON T.CODPROD = P.CODPROD
  LEFT JOIN TRANSF_ES T2 ON T2.CODPROD = P.CODPROD
