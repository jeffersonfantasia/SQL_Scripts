CREATE MATERIALIZED VIEW MV_JC_BASE_BAIXA_CRED
...
AS
  WITH ADIANT_REALIZADOS_VERIFICA_MES AS
   (
    ...
    SELECT C.CODIGO,
            (CASE
              WHEN ((TO_NUMBER(TO_CHAR(C.DTDESCONTO, 'YYYY') ||
                               TO_CHAR(C.DTDESCONTO, 'MM')) <>
                   TO_NUMBER(TO_CHAR(C.DTLANC, 'YYYY') ||
                               TO_CHAR(C.DTLANC, 'MM'))) AND
                   C.DTESTORNO IS NOT NULL) THEN
               1
              ELSE
               0
